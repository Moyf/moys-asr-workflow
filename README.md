...
Launcher 的后处理工具箱可选用 DeepSeek、阿里云 Qwen 或自定义 OpenAI-compatible 服务校对、重分句或翻译字幕。该步骤只发送带临时 ID 的字幕文字，不发送时间码、媒体路径或工程元数据；选择供应商即表示字幕文字会按该服务商的条款和隐私政策传输。后处理 Key 只保存在本机 `.env`，不会写入工程、SRT 或日志。
