...
> Launcher 已提供实验性的本地 Qwen3-ASR / FunASR 识别方式（Beta），见 [docs/LOCAL_ASR.md](docs/LOCAL_ASR.md)。当前为 beta 版本，未经过充分测试，不保证后续的维护和更新，请谨慎使用。Windows 打包版可通过 GUI 安装本地运行环境和模型；安装包本身仍不携带 Torch 或模型权重。
