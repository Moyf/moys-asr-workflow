...
- 新增按钮、输入框、提示行等 UI 元素时，必须与上下相邻元素保持垂直间隔（用 `margin` 或父容器 `gap`），不得紧贴；移动、新增一行按钮或区块后，要检查它与上方元素的间距，用浏览器截图或实测确认，不能只看代码。
- 这条规则反复被违反过：凡是改动 Launcher / 编辑器布局的任务，收尾前把涉及区域截图自查一遍。
