...
- **ReaPeaks 波形与频谱染色的时间轴错位** ： ReaPeaks 派生波形的每秒峰数此前被取整成整数当刻度使用，而它多数情况下是分数（16 kHz 媒体为 301.8868），于是整条波形被按比例缩放、错位随播放位置线性累积（实测 15 分钟视频末端错开约 0.33 秒，并与染色层逐帧错开）。现在波形载荷同时给出精确的 `sample_rate` / `division`，绘制、空隙检测和浏览器端解析一律改用真率，`duration_ms` 与之保持自洽；44.1 / 48 kHz 等能整除的媒体行为不变。
- **立体声 ReaPeaks 波形只画了第一声道** ： 服务端派生波形载荷此前固定取声道 0，而浏览器直接解析 `.ReaPeaks` 时合并全部声道，同一份文件在两个入口得到不同轮廓；双单声道素材（人声只在右声道）在服务端会画成一条直线。现在统一为合并各声道极值。
- **按音量移除空隙的判断依据与显示不符** ： 该检测此前永远读取自研波形缓存，而默认显示的是 ReaPeaks 形状；自研链先重采样到 1000 Hz，带限之外的瞬态会被整块削平，用它做静音门限偏激进。现在检测与绘制共用当前实际显示的那条包络。

### 🔄 变更

- **ReaPeaks 缓存来源收紧** ： `.ReaPeaks` 与内嵌波形改为始终从工程记录的源媒体本身解码，不再复用本地 ASR 的 16 kHz 单声道提取音频或 `--length-limit` 截断片段，因此缓存覆盖完整媒体并保留原始采样率与声道；缓存有效性同时校验媒体的修改时间与字节大小，此前遗留的“派生文件缓存”会被判定过期并重建（首次打开可能多花一次解码的时间）。
