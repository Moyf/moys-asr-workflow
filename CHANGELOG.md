...
- **本地对齐与时间码工具箱** ： 新增 FireRedASR2 CPU 本地识别，固定串联 FireRedASR2-CTC 与现有本地运行环境中的 FunASR `ct-punc`，按标点重新断句并保留原始字词时间码；Qwen3-ForcedAligner 与 FireRed CTC 可作为对齐模型独立下载，其中 Qwen 对齐器复用 Qwen Local 的模型缓存；MOSS 以及 SRT / MOSP 后处理现在可以生成或补充字词级时间码。本地模型统一在「设置 → AI 模型配置」管理，ASR 与对齐模型分组显示，并只为没有原生字词时间码的 ASR 模型提供对齐模型选项；本地 ASR 分组列出全部可用本地模型及其准备状态，可直接切换查看；本地模型运行时设置增加可展开的运行时清单，可查看版本、Python 版本和依赖安装情况。
