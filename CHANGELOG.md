...
- **字幕预览说话人标签** ： 可为黄、绿、红、紫、蓝五种字幕颜色配置并重命名说话人名称（默认 `SP1`～`SP5`）；开启后仅在播放器预览前显示标签，不改动工程字幕文本，导出 SRT 时可单独选择是否附加。
- **主副字幕交换的颜色映射** ： 交换时按绑定关系将主字幕颜色映射到新的主轨，并保留移到副字幕的原主轨 `color` / `color_ref` 信息；颜色引用的 `headIdx` 会按各自字幕数组重新计算。

### 🔄 变更

- **波形缓存改为二进制容器，移除 JSON sidecar** ： 转写生成的 peaks 容器由 REAPER 的 `<媒体>.ReaPeaks` 改为自有 `<媒体>.quapeaks`（内含 wave / spectral / loudness 与新增的**自研波形层**），编辑器按需提取的波形缓存由 `<媒体>.waveform.json` 改为二进制
  `<媒体>.mopeaks`。两者与 `.ReaPeaks` 共用同一套 18 字节全局头与层表布局，非默认音轨统一带 `.track-N` 段。**旧的 `waveform.json` 既不再生成也不再读取**：升级后首次打开会重新提取一次波形，此后可安全删除历史文件。REAPER 原生 `.ReaPeaks` 继续照读。
- **Rust peaks 内核换为 PyPI 包 `quapeaks`** ： 依赖从 `reapeaks` 切到 `quapeaks>=2026.0.0`，`maw/reapeaks.py` 更名为 `maw/quapeaks.py`；导入内核失败时仍打日志跳过缓存生成，不阻断转写入口。

### 🐛 修复

- **派生音频回退时波形签名不再张冠李戴** ： 源媒体不可解、改用派生 WAV 生成缓存时，回退缓存现在写在**真正解码过的那个文件**旁并记它的签名（源与派生同名不同目录时也不再写错位置），避免短片段的时间轴被当成完整源媒体的有效波形接受。
- **损坏的波形缓存不再让编辑器崩溃** ： 缓存刻度非法（`division` 为 0 等）、magic 版本不认识或峰数损坏时一律按“无缓存”降级重抽，而不是在耗时转写完成后抛出未捕获异常。
- **浏览器可导入自有容器** ： 波形形状来源与手动导入识别 `.quapeaks` 与 `QPK*` magic，此前只有服务端读得到，用户在浏览器里打开会报“不支持的文件”。
