...
- **字幕预览说话人标签与颜色样式补充** ： 可为五种字幕颜色配置并重命名说话人名称；预览标签继承主字幕字体和对应颜色，导出 SRT 时可单独选择是否附加名称与分隔符；预览颜色支持下划线、文字颜色和组合样式。
- **全局设置窗口缩放与媒体布局** ： 设置窗口支持拖动右下角调整并记忆尺寸；媒体预览区和布局编辑模式收窄冗余留白，拖动分隔线仅在悬停或拖动时显示。
...
- **MOSE Electron 独立编辑器与 Windows 套件 ZIP** ：恢复桌面编辑器发布路线，Windows x64 新增 `MAW-MOSE-Windows-x64-<version>.zip`；Electron 通过同套件 `MAW.exe` 启动受令牌保护的随机端口 Server，与 MAWE 共用保存、波形、媒体、最近工程和导出能力。Launcher 主按钮优先打开 `MAW\MOSE\MOSE.exe`，缺失或启动失败自动回退 Server，并仅为当前用户建立 `.mosp` 关联。
- **官方 Windows Installer 与更新基础** ：新增无需管理员权限的 Windows x64 Installer、稳定 / Beta 频道检查、ETag 缓存、Release 清单与 SHA-256 校验；Installer 统一包含 MAW + MOSE，`.mosp` 双击先经过 Launcher 更新检查再打开 MOSE，并随公开 GitHub Release 发布。更新过程会保留用户配置、日志和模型缓存。
