...
- **字幕预览说话人标签与颜色样式** ： 新增独立的「字幕颜色」设置页；开启「将颜色映射为说话人」后，可配置黄、绿、红、紫、蓝五种字幕颜色对应的说话人名称（默认 `SP1`～`SP5`），并选择是否在播放器预览字幕前显示标签。名称使用对应字幕颜色并继承主字幕字体，不改动工程字幕文本；名称元素包含可自定义的分隔符（默认 `：`，支持空格或英文引号），导出 SRT 时可单独选择是否附加名称和分隔符；按颜色导出时还可使用说话人名称替代颜色作为文件名后缀；导出提示中的「字幕颜色」可直接点击跳转到对应设置，颜色样式选择器使用紧凑宽度；预览字幕颜色支持互斥的下划线、文字颜色和描边样式，空隙 click 不再提前显示拖动样式。
