...
- **字幕预览说话人标签与颜色样式** ： 可为黄、绿、红、紫、蓝五种字幕颜色配置并重命名说话人名称（默认 `SP1`～`SP5`）；开启后仅在播放器预览的字幕前以独立元素显示标签，名称使用对应字幕颜色并继承主字幕字体，不改动工程字幕文本；名称元素包含可自定义的分隔符（默认 `：`，支持空格或英文引号），导出 SRT 时可单独选择是否附加名称和分隔符；预览字幕颜色支持互斥的下划线、文字颜色、阴影和描边样式，空隙 click 不再提前显示拖动样式。
