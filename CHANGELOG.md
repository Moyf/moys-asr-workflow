...
- **豆包语音识别供应商（火山引擎）** ： 新增豆包大模型录音文件识别作为云端转写供应商，单一 `VOLC_API_KEY`（火山引擎新版控制台）即可使用，Launcher 与公开 CLI（`--provider doubao`）均可选择；支持字/词级毫秒时间戳、可选说话人分离与颜色映射、即时热词直传（`corpus.context`），并可通过 `VOLC_ASR_RESOURCE_ID` 切换 2.0（默认 Seed-ASR）/ 1.0 / 闲时版模型。转写前自动把媒体提取为 ogg + opus 24kbps 单声道音频再 base64 直传，适配官方「单文件 ≤25MB 且 ≤120 分钟」的限制，超限任务会给出明确的截取/分割提示。
...
- **服务器工程加载进度** ： 启动时区分读取波形缓存与实际生成波形，并分别反馈波形、频谱缓存和工程收尾阶段，命中缓存时不再显示「正在生成波形」。
...
- **文稿匹配预览与错误提示** ： 文稿匹配新增默认开启的 Markdown 符号清理，并让文稿预览、拆分预览、手动匹配与自动后处理共用断句、句尾标点和清理设置；默认不再把逗号、句号写入匹配结果。匹配产物按界面语言使用 `匹配` / `match` 后缀；非法字幕/工程解析、文稿解析和低匹配度会分别提示并在重新计算后刷新预览。
...
- **提示卡片被浮层遮挡** ： 右上角操作提示现在会显示在设置窗口与导出菜单上方，避免 toast 被打开的浮层盖住。
- **布局分隔条初始样式** ： 编辑器首次打开时也会立即使用变窄、默认隐藏的分隔条样式，悬停或拖动时再显示，不必先点击「编辑布局」。
