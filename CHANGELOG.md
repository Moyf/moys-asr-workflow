...
- **本地运行环境安装失败不再永久卡在「安装中」** ： 安装失败、取消或进程退出后残留的 `runtime.json` 安装标记现在会自动改写：查询状态时自愈，取消与失败分支也会主动清理，「重新扫描」和「修复」随时可用，无需再手动删除 `runtime.json`（OCR 侧已有同类恢复，本次补齐本地与 MOSS 侧）。同时 PyTorch 轮子源（`download.pytorch.org`）纳入安装前可达性探测：不可达时立即给出明确提示，不再让安装挂上几小时后以无关依赖（如 `msgpack`）的模糊版本解析错误收场；该源也可用环境变量 `MAW_PYTORCH_INDEX` 整源替换为镜像，见文档「安装源与环境变量」。
