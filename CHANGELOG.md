...
- **重叠 SRT 字幕轨** ： 支持双层重叠 SRT 导入为「叠加字幕」独立轨，与主字幕时间重叠、并排编辑，可标记颜色/表情包、拆分合并、禁用、撤销重做与播放预览，并随字幕（SRT/ASS/分色）、FCP7 XML 与 OTIO 导出；主轨与叠加轨可互相转换（颜色/表情包标记跟随）；第三层重叠会明确报错。叠加轨字幕行的序号徽标在英文界面显示为 `OVL` 前缀（如 `OVL1`）。启用「叠加字幕」轨后，在已有主字幕的位置按住 `Ctrl/Cmd` 拖动波形（或空白波形右键「创建叠加字幕」）可直接在叠加轨创建与主字幕重叠的新字幕；`Shift`+拖动主字幕遇到邻居遮挡时自动转入叠加轨继续移动，拖回无重叠位置自动放回主轨。ASS 导出中叠加字幕以独立层渲染在主字幕上方，主字幕底边距相应由 40px 调整为 80px。
