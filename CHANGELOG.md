...
- **毫秒 / 帧时间基准** ： 编辑器支持按工程设置 FPS 切换毫秒或帧时间轴；帧模式下字幕拖动、边界调整、方向键和 A/D 微调均按帧执行，并以 `HH:MM:SS:FF` 显示时间码，同时保留毫秒兼容字段；全局设置还可配置帧吸附和时间码分隔符，帧吸附默认开启但仅在帧模式生效。生成 `.mosp` 时会尽力记录源视频 FPS，打开旧工程时缺失该字段仍使用原有默认值。
- **帧时间基准转换确认** ： 切换到帧模式或在帧模式修改 FPS 前，会提示批量映射时间码及原始毫秒值不可恢复的影响。
- **波形细网格** ： 波形最大缩放扩展为 2 秒，多行波形也新增 2 秒每行长度；仅在基础模式 2 秒显示窗口或多行模式 2 秒每行长度下按当前时间基准绘制细分竖线，毫秒模式每 100ms 一格、帧模式按当前 FPS 对齐，且颜色弱于秒分隔线。
