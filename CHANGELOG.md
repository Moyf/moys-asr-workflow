...
- 简繁转换支持通用繁体、台湾繁体和台湾增强繁体三种目标模式，并在可安全对应时尽量保留字词时间码。
- 简繁转换补充香港繁体目标模式。
- 修复 SRT 编码兼容：导出的 SRT 使用带 BOM 的 UTF-8，导入支持 UTF-8、UTF-16 和 GB18030 / GBK 字幕文件，减少在 PR 等工具中出现中文乱码的情况。
- PR #57：编辑器新增默认关闭的暂停时波形悬停画面预览，并将媒体预览、JKL 播放控制和跳转时长整合到同一设置分组；补充便携产物、Playwright 覆盖及相关 CLI 测试输出整理。
...
- **ReaPeaks 签名修复** : 修复 GUI 转写场景下 `.ReaPeaks` 头部记录的是临时提取音频的大小、而服务器按源媒体校验导致缓存被误判失效、频谱染色与 ReaPeaks 波形切换失效的问题；校验降级为仅比对秒级 mtime，size 字段不再参与匹配，缓存始终写到源媒体旁（含测试模式限长提取场景）。
