...
- **波形设置精简与副字幕 192px 行高** ： 波形区 ⚙️ 面板中的操作类设置（允许拖动指针、贴合字幕边界、自动吸附调整相邻字幕、快捷键时间基准、字幕按键微调幅度、空隙区段操作方式）移入「全局设置 → 通用操作」，波形 ⚙️ 面板只保留波形样式外观，其中「禁用波形」更名为「禁用波形显示」并保留在波形 ⚙️ 面板；帮助中的相关入口同步指向新位置。双语字幕设置中的「副字幕时波形高度」补上与主波形一致的 192 px 档位。
- **「使用 ASS 样式」旁新增「编辑样式」** ： ASS 字幕模式下，「字幕样式」页主字幕和副字幕的样式下拉旁提供「编辑样式」按钮，点击直接打开样式库窗口并定位到当前选中的样式。
