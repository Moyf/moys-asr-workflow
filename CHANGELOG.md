...
- **ASS 样式库与预览模式** ： Editor 新增默认关闭的「ASS 字幕模式」和独立样式管理窗口，支持用户级样式、ASS 输出方案、默认槽位，以及颜色、字体、粗体、斜体、下划线、描边、阴影、对齐等 ASS 样式；ASS 导出读取默认 ASS 方案，并可逐句添加 `\fad`、`\fade`、`\move`、`\t`，Launcher 压制 SRT 读取 SRT 默认样式。localhost Editor 与 Launcher 共享本机用户级配置，便携 `file://` Editor 使用浏览器本地副本。字体设置改为文本输入 + 可筛选下拉列表（按显示名去重，支持本机字体扫描）；启用「ASS 字幕模式」后「预览字幕颜色」由 ASS 接管并给出跳转提示；默认 ASS 样式按操作系统选择中文字体（Windows 微软雅黑 / macOS 苹方 / 其他 Noto Sans CJK SC）并默认加粗；内置「ASS」样式与方案更名为「ASS 默认样式」「ASS 输出方案」，用户自定义过的名称不受影响。ASS 预览在小窗、全屏和舞台尺寸变化时按 PlayRes 重新计算，主字幕与说话人标签保持相同的字体和样式。多重字幕的副字幕随 ASS 导出与预览：所有副字幕共用一个「ASS 副字幕样式」内置槽位（可在样式库中改选任意样式，边距、对齐、字号完全由该样式决定），ASS 字幕模式下「字幕样式」设置的主/副字幕 CSS 控件会替换为样式库选择器并与样式库同步。与「叠加字幕」轨共用时，叠加字幕导出为独立的 Overlay 样式（字段与主字幕一致，锚定边距固化为「下方最近一层字幕的边距 + 1.2 倍字号」，在两层文字间留出空档；有副字幕时自动叠在副字幕上方），颜色映射与主字幕一致，并逐句应用 `\fad`、`\fade`、`\t`（`\move` 的绝对坐标只属于主字幕）。样式库的删除操作跟随当前表单标题，SRT 默认样式表单补充烧录说明，默认槽位选择器补充用途提示，ASS 样式表单显示 ASS 模式启用状态，左侧列表保持简洁。
