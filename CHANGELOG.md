...
- 完成后日志与结果中会显示烧录出的视频路径；批量转写的结果里同样带出 `videoPath`。

### ✨ 提升

- 手动「烧录字幕」新增「视频编码器」选择：自动（优先硬件）/ CPU（libx264）/ NVIDIA NVENC / AMD AMF / Intel QSV；自动模式会探测当前 FFmpeg 可用的硬件编码器并按序尝试，全部失败时回退 CPU，可在工具箱中实时看到 FFmpeg 进度日志。
