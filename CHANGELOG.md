...
- **贴合字幕边界拖动新模式（默认）** ： 两条字幕贴合时，中缝处新增可拖动的边界区，拖动中缝两侧边界一起联动；单独拖动左右手柄仍只调整当前字幕（类似达芬奇的剪辑逻辑）。左右手柄悬停显示开始／结束时间的操作提示，左右手柄使用单竖线配单向箭头光标，中缝使用双竖线配双向箭头光标，统一样式并区分单侧与双侧联动调节。旧的「自动吸附调整相邻字幕」开关移入「传统模式」，可在「音频波形区 ⚙️ → 操作 → 贴合字幕边界」中切换；键盘微调的默认联动规则不变。
