...
- **OpenRouter OpenAI 兼容 ASR** ： Launcher 新增 OpenRouter 可用的 OpenAI 转写模型与参考价格；使用 OpenRouter 时，内置模型会自动补全 `openai/` 模型前缀，并在 API Key 入口提供 OpenRouter。其他云端 API 模型也会显示服务商参考价格；其他中转站会提示用户选择“自定义（Custom）”并填写服务商的完整模型名。
- **OpenAI 转写高级设置** ： 按模型显示并转发 Prompt、Keywords；新增官方 `gpt-transcribe` 和 `gpt-4o-transcribe-diarize` 选项。说话人模型会自动请求 `diarized_json` 并保留 speaker 标签，OpenRouter 会明确提示不支持 diarize。
...
- **字幕预览说话人标签与颜色样式** ： 可为黄、绿、红、紫、蓝五种字幕颜色配置并重命名说话人名称（默认 `SP1`～`SP5`）；开启后仅在播放器预览的字幕前以独立元素显示标签，名称使用对应字幕颜色并继承主字幕字体，不改动工程字幕文本；名称元素包含可自定义的分隔符（默认 `：`，支持空格或英文引号），导出 SRT 时可单独选择是否附加名称和分隔符；预览字幕颜色支持互斥的下划线、文字颜色和描边样式，空隙 click 不再提前显示拖动样式。
...
- **工程版本备份** ： Editor「保存」设置新增默认开启的工程备份，本地 Server 绑定工程可按分钟定时备份，手动保存也创建带时间戳的 `.mosp-bak`；备份放入工程对应的 `_maw/backups`，可设置最多保留的版本数，超出后将最早版本移入回收站。备份不加入「最近工程」，可点击「打开备份文件夹」手动恢复。
- **字幕预览说话人标签与颜色样式** ： 新增独立的「字幕颜色」设置页；开启「将颜色映射为说话人」后，可配置黄、绿、红、紫、蓝五种字幕颜色对应的说话人名称（默认 `SP1`～`SP5`），并选择是否在播放器预览字幕前显示标签。名称使用对应字幕颜色并继承主字幕字体，不改动工程字幕文本；名称元素包含可自定义的分隔符（默认 `：`，支持空格或英文引号），导出 SRT 时可单独选择是否附加名称和分隔符；按颜色导出时还可使用说话人名称替代颜色作为文件名后缀；导出提示中的「字幕颜色」可直接点击跳转到对应设置，颜色样式选择器使用紧凑宽度；预览字幕颜色支持互斥的下划线、文字颜色和描边样式，使用描边时说话人标签改用主字幕颜色填充，避免与描边同色；空隙 click 不再提前显示拖动样式。
- **「视频预览」与「字幕样式」设置分区** ： 全局设置的「预览字幕」标签页升级为「视频预览」，预览开关组保留预览字幕 / 预览副字幕 / 预览表情包，播放控制组包含自动预览鼠标位置画面、JKL 播放模式与每次跳转时长（均自「通用操作」移入）；原「字幕预览」样式设置独立为「字幕样式」标签页，并按「主字幕」与「副字幕」拆分为两个设置组，其中「字幕大小」更名「文字大小」，字体与文字大小各自成行，各设置项样式与窗口其他分区统一；颜色预览和说话人映射独立为「字幕颜色」标签页，「预览字幕颜色」不再提供「阴影」样式，旧工程中保存的阴影自动回退为下划线。全局设置左侧标签按「基础」（界面 / 通用操作）、「媒体」（视频预览 / 字幕样式 / 字幕颜色）、「编辑」（时间基准 / 拆分与合并）、「工程与输出」（导出 / 保存）和「扩展功能」（表情包 / 彩蛋）分组；「界面」页提供语言切换与「明亮模式 / 暗色模式 / 跟随系统」三态主题，并将主题选择改为更清晰的实心 Active 状态。
- **全局设置窗口缩放** ： 全局设置窗口与帮助窗口一致，可拖动右下角调整窗口大小，并记忆上次尺寸。
- **浮动窗口层级** ： 全局设置、帮助、工具窗以及设置/工具栏齿轮弹窗按最后激活顺序自动置顶，避免被先打开的浮层遮挡；从全局设置窗口打开的「表情包根目录」弹窗也入栈置顶，打开时始终盖住设置窗口。
- **媒体区与布局编辑视觉** ： 移除媒体预览区顶部的「媒体」标签栏（其功能已并入全局设置，「视频预览」分区可直接管理），画面区域更高；布局编辑模式的拖动分隔线默认隐形、悬停或拖动时才显示，线条从 3px 收窄为 2px；「字幕编辑」区不再常驻顶部留白，进入布局编辑时与媒体区一样自动让出拖动把手高度；当前字幕编辑区移除左侧强调边框。
- **主副字幕交换的颜色映射** ： 交换时按绑定关系将主字幕颜色映射到新的主轨，并保留移到副字幕的原主轨 `color` / `color_ref` 信息；颜色引用的 `headIdx` 会按各自字幕数组重新计算。
- **分色导出文件名中文化** ： 编辑器界面语言为中文时，按颜色导出的 SRT 文件名颜色段同步使用中文（如 `clip_红色.srt`、`clip_去空隙_蓝色.srt`），未标记颜色的默认组仍为 `default`；英文界面保持原样。
- **服务器工程启动提示** ： 本地 Server 编辑器在后台读取工程、准备媒体和生成波形期间，首屏直接显示「正在加载工程…」及进度状态，避免空编辑器看起来像加载失败。
- **界面强调色** ： 全局设置「基础 → 界面 → 外观」新增蓝色、红色、橙色和自定义强调色选项；选择自定义时可以直接取色，拖动取色器会延迟应用以避免频繁重绘，界面中的活动、悬停、聚焦和提示样式会同步换色。亮色主题下主字幕波形块改为白底黑字，副字幕使用较浅的灰底以便区分；有颜色映射的主字幕块仍保留对应的字幕颜色。
...
- **波形数据不再内联进工程文件** ： `.mosp` / 工程 `.json` 只保存字幕、设置与当前所选音轨，工程体积不再随素材时长暴涨；波形 payload 仅保留在运行态供页面即时渲染，落盘真源为媒体旁的 `.quapeaks` / `.mopeaks`。旧工程里已有的内联缓存继续可用，保存后自然移除。
...
- **多重字幕下隐藏「仅看超长」** ： 「仅看超长」按单轨文本字数筛选，多重字幕双列下主/副两栏合并计数失去筛选意义，开启多重字幕时隐藏该按钮及其前置分隔线；若筛选已激活会自动复位，避免残留不可见的过滤状态。
- **说话人名称输入框重叠** ： 修复全局设置「显示说话人名称」中的名称与分隔符输入框使用固定宽度、在较窄的设置窗口下互相重叠的问题；输入框现在跟随所在列宽动态伸缩。
- **窄列表时间码箭头溢出** ： 字幕列表变窄切换为两行时间码时隐藏中间箭头（上下排列已表达先后顺序），帧模式等 11 字符时间码不再超出预留列宽挤压相邻内容。
- **云端转写降级不再放弃断句** ： 当云端返回中部分句子缺少可用的词级时间码（时间码缺失、倒挂或与句文本不匹配）时，此前整份结果会跳过全部切句逻辑，直接保留服务端段落边界，输出大量几十字、带逗号却不拆分的超长字幕；现在改为保留服务端句边界并对超长段二次拆分——有词级时间码的段仍按词级精确拆分，没有的段按标点切块并在句段范围内按字数占比插值时间，保证「最大字数」「停顿切句」与断句符号在降级场景下依然生效；Qwen-Audio / Fun-ASR 句级结果中无词级时间码的超长句同样受益。
- **波形缓存读取更稳健** ： 修复派生音频缓存签名与落点错误；多音轨媒体按容器默认轨使用无后缀缓存，所选轨会随工程恢复，批量转写也会分别识别每个媒体的默认轨；损坏、不支持版本或缺少所选轨精确缓存时可安全降级并重建。
- **浏览器可导入自有容器** ： 波形形状来源与手动导入现在支持 `.quapeaks`；此前只有服务端能够读取。
- **设置页签嵌套错误** ： 修复「导出」设置页少一个闭合标签，导致「保存 / 表情包 / 彩蛋」三个页签切换后内容空白的问题。
- **帮助打开设置被遮挡** ： 从帮助窗口点击「全局设置」打开设置窗口时，窗口不再被帮助窗口压住（表情包根目录弹窗仍保持在最上层）。
...
- **重叠 SRT 字幕轨** ： 导入含两层重叠 cue 的 SRT 时保留独立叠加轨；工程、Server 保存和合并 SRT 导出均保留两条轨道，第三层重叠会明确提示无法导入。编辑器内可把选中主轨字幕通过右键「转为叠加字幕」迁入叠加轨（列表与波形均可右键，支持编辑、转回主轨、删除与撤销）；叠加字幕在波形上显示于主字幕块正上方的独立一层（多重字幕时为主字幕与副字幕之上的第三层），并接入播放器字幕预览；主轨与叠加轨互转时颜色标记与表情包跟随字幕（引用物化为叠加轨自持 head，剩余组员按组拆分语义提升新 head），叠加轨预览文本同样应用颜色快照与说话人标签样式，表情包支持在叠加轨分配、预览与随播放显示；导出侧同步打通：ASS 以独立图层（顶部居中）写入叠加字幕、可与主轨时间重叠，按颜色拆分 SRT 与说话人标签均覆盖叠加段，FCP7 XML / OTIO 可将叠加轨字幕与表情包导出为独立轨道；按住 Shift 拖动主轨字幕时，被相邻字幕挡住会自动转入叠加轨继续移动，拖回不重叠的位置会自动转回主轨，按住 Shift 拖动叠加字幕同理放回主轨，松手时停在当前所在轨道，换轨后选中状态与字幕面板跟随被拖字幕；叠加字幕支持拆分（面板、右键菜单、剃刀工具，复用副轨拆分弹窗）与相邻合并（`Ctrl/Cmd+Shift+A/D`），颜色/表情包组随拆分合并按主轨同款规则继承；叠加字幕与主字幕共用标记颜色（数字键 1~5 / 0）、表情包分配（T）与禁用切换（Alt+点击）等交互；多重字幕开启时以上能力均可用。
