...
- **ASS 样式库样式方案表单重构** ： ASS 输出方案表单中「使用样式」改为「主字幕样式」，开启多重字幕时右侧显示「副字幕默认样式」（即原默认槽位中的副字幕样式，不再单独配置）；`\fade`、`\move`、`\t` 收进「进阶动画」折叠分组，勾选框直接显示在名称旁（去掉「启用」文字），α 与 t 参数分行排列，`\t` 的「ASS 标签」更名「自定义 ASS 标签」并加大输入框、启用时以强调色描边提示；「当前方案」摘要卡背景跟随明暗模式，修复亮色模式下文字不可读。
- **导入字幕弹窗布局** ： 「替换当前字幕 / 作为副字幕」（原「作为双语字幕」）改为预览框上方的左侧 tabs，「取消 / 导入」按钮居中。
- **重叠 SRT 字幕轨** ： 支持双层重叠 SRT 导入为「叠加字幕」独立轨，与主字幕时间重叠、并排编辑，可标记颜色/表情包、拆分合并、禁用、撤销重做与播放预览，并随字幕（SRT/ASS/分色）、FCP7 XML 与 OTIO 导出；主轨与叠加轨可互相转换（颜色/表情包标记跟随）；第三层重叠会明确报错。启用「叠加字幕」轨后，在已有主字幕的位置按住 `Ctrl/Cmd` 拖动波形（或空白波形右键「创建叠加字幕」）可直接在叠加轨创建与主字幕重叠的新字幕；`Shift`+拖动主字幕遇到邻居遮挡时自动转入叠加轨继续移动，拖回无重叠位置自动放回主轨。ASS 导出中叠加字幕以独立层渲染在主字幕上方，主字幕底边距相应由 40px 调整为 80px。
- **ASS 样式库与预览模式** ： Editor 新增默认关闭的「ASS 字幕模式」和独立样式管理窗口，支持用户级样式、ASS 输出方案、默认槽位，以及颜色、字体、粗体、斜体、下划线、描边、阴影、对齐等 ASS 样式；ASS 导出读取默认 ASS 方案，并可逐句添加 `\fad`、`\fade`、`\move`、`\t`，Launcher 压制 SRT 读取 SRT 默认样式。localhost Editor 与 Launcher 共享本机用户级配置，便携 `file://` Editor 使用浏览器本地副本。字体设置改为文本输入 + 可筛选下拉列表（按显示名去重，支持本机字体扫描）；启用「ASS 字幕模式」后「预览字幕颜色」由 ASS 接管并给出跳转提示；默认 ASS 样式按操作系统选择中文字体（Windows 微软雅黑 / macOS 苹方 / 其他 Noto Sans CJK SC）并默认加粗；内置「ASS」样式与方案更名为「ASS 默认样式」「ASS 输出方案」，用户自定义过的名称不受影响。ASS 预览在小窗、全屏和舞台尺寸变化时按 PlayRes 重新计算，主字幕与说话人标签保持相同的字体和样式。多重字幕的副字幕随 ASS 导出与预览：所有副字幕共用一个「ASS 副字幕样式」内置槽位（可在样式库中改选任意样式，边距、对齐、字号完全由该样式决定），ASS 字幕模式下「字幕样式」设置的主/副字幕 CSS 控件会替换为样式库选择器并与样式库同步；多重字幕关闭时副字幕不再随 ASS 预览与导出，去空隙 ASS 导出也包含副字幕与叠加轨。与「叠加字幕」轨共用时，叠加字幕导出为独立的 Overlay 样式（字段与主字幕一致，锚定边距固化为「下方最近一层字幕的边距 + 1.2 倍字号」，在两层文字间留出空档；有副字幕时自动叠在副字幕上方，并沿用其锚定层的对齐与水平边距），颜色映射与主字幕一致，并逐句应用 `\fad`、`\fade`、`\t`（`\move` 的绝对坐标只属于主字幕）。样式库的删除操作跟随当前表单标题，SRT 默认样式表单补充烧录说明，默认槽位选择器补充用途提示，ASS 样式表单显示 ASS 模式启用状态，左侧列表保持简洁。
...
- **ASS 样式库小字与列表滚动** ： 样式库内不超过 10px 的字号统一提升到 12px（徽标、分区标题、单位、注脚等），点击列表项按下时不再因按压位移挤出滚动条；样式表单的「对齐」九宫格放大到与边距、缩放两栏等高并三列均分，「加粗」移到「斜体」前同一行，隐藏暂未支持的「次色」（\k 卡拉 OK 标签），字体旁按钮改为「读取本机字体」（与设置页共用扫描，结果直接进入字体下拉）。
