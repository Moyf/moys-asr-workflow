...
- **本地模型安装** ： 修复有 NVIDIA 显卡的电脑安装 / 修复本地运行环境时，因依赖清单缺少 sherpa-onnx 上游新拆分的 `sherpa-onnx-core` 条目（含各平台哈希）而安装失败的问题。
- **英文界面** ： 补齐英文词典缺失的 4 条词条（SRT 烧录、读取本机字体提示、次色说明、禁用波形显示），次色提示中的卡拉 OK 标签现在正确显示为 `\k`。
- **颜色组脱离** ： 恢复右键菜单中的「从颜色组中脱离」：字幕可保留当前颜色并脱离颜色组，原组其余字幕自动维持有效分组（该功能在上一次 ASS 样式库重构中遗失）。
