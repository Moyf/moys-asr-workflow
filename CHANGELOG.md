...
- **在叠加轨上直接创建字幕** ： 启用「叠加字幕」轨后，在已有主字幕的位置按住 `Ctrl/Cmd` 拖动波形（包括直接在主字幕块上按住拖动）会改为在叠加轨创建，新字幕与主字幕时间重叠存放，创建虚影显示在叠加轨 lane；空白波形右键菜单同步新增「创建叠加字幕」入口。同一操作在空白处仍创建主字幕，叠加轨同轨仍不允许重叠。
- **叠加字幕菜单项「转为主字幕」** ： 叠加字幕右键菜单中的「转回主轨」更名为「转为主字幕」；主字幕轨同一时间段已有字幕时该项置灰禁用，避免转回后产生主轨重叠。

### 🐛 修复

- **拖动字幕实时跟随修复** ： 工程中存在颜色／表情包徽章时，拖动字幕（尤其叠加字幕）过程中波形块位置不再实时跟随、松手后才跳到新位置；边界拖动同样受影响。现在拖动全程实时刷新，徽章也随字幕块同步移动。
