...
- **毫秒 / 帧时间基准** ： 编辑器支持按工程设置 FPS 在毫秒与帧时间轴间切换；帧模式下拖动、边界调整、方向键和 A/D 微调均按帧执行，并以可配置分隔符显示 `HH:MM:SS:FF`，同时保留毫秒兼容字段与媒体 FPS 元数据。
- **波形细网格** ： 波形支持 2 秒视窗和每行长度，并在细化视图中按当前时间基准绘制帧网格。

### ✨ 提升

- **Launcher 首屏防闪烁与启动优化** ： 不再插入独立启动页；正式页面只等待轻量配置和基础组件初始化，运行环境、模型缓存、OCR、FFmpeg 与已有服务检查改在页面显示后异步执行，减少 WebView2 首帧白底闪烁和大缓存扫描造成的启动等待。
...
- **LLM 后处理可靠性** ： 改进 OpenAI 兼容服务的 HTTP、网络和协议错误提示，并自动脱敏 API Key 与服务地址；翻译遇到模型漏答时进行有限补救，失败后保留原始转写并可从失败步骤重试，避免把供应商响应误报为网络故障。
- **本地 ASR 语言与切句粒度** ： MOSS 段级时间码不再被伪造成字符/单词时间码或硬切成错误字幕；所有转写工程统一记录语言来源、字符型/单词型切句模式和时间码粒度，并在 Launcher 暴露英文最大单词数（默认 13）与短句合并阈值（默认 3）。MOSS 转写期间还会在 Launcher 日志中显示实时已生成 token 数；由于输出总长度不可预知，不显示伪造的百分比。
- **双语字幕产物保护** ： 合并双语时跳过空 cue、清理旧版副轨字段，并为手动与自动产物添加 `.bilingual` 标记；翻译入口同时检查工程和 SRT，避免再次翻译合并结果。
- **大媒体 ReaPeaks 生成** ： 头部 mtime/size 指纹按官方规格对齐为 stat() 值的低 32 位（内核 ≥0.3.2），修复 >2GiB 的长视频「生成波形」时报 ReaPeaks cache was not generated 的问题；缓存校验同时容忍跨盘拷贝造成的秒级与夏令时整小时 mtime 漂移，不再误判缓存失效。
- **MOSS 本地转写启动即失败** ： 转写入口不再经由便携编辑器生成器导入 Rust `.ReaPeaks` 内核，未安装该内核的托管运行环境（MOSS）不再在加载模型前报 `ModuleNotFoundError`；内核确实缺失时改为打日志跳过 `.ReaPeaks` 缓存生成，字幕、`.mosp` 工程和自研波形照常输出。
