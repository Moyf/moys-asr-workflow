...
- **重叠 SRT 字幕轨** ： 导入含两层重叠 cue 的 SRT 时保留独立叠加轨；工程、Server 保存和合并 SRT 导出均保留两条轨道，第三层重叠会明确提示无法导入。编辑器内可把选中主轨字幕通过右键「转为叠加字幕」迁入叠加轨（列表与波形均可右键，支持编辑、转回主轨、删除与撤销）；叠加字幕在波形上显示于主字幕块正上方的独立一层（多重字幕时为主字幕与副字幕之上的第三层），并接入播放器字幕预览；按住 Shift 拖动主轨字幕时，被相邻字幕挡住会自动转入叠加轨继续移动，拖回不重叠的位置会自动转回主轨，按住 Shift 拖动叠加字幕同理放回主轨，松手时停在当前所在轨道，换轨后选中状态与字幕面板跟随被拖字幕；多重字幕开启时以上能力均可用。
