...
- **波形数据不再内联进工程文件** ： `.mosp` / 工程 `.json` 只保存字幕与设置，工程体积不再随素材时长暴涨；波形 payload 仅保留在运行态供页面即时渲染，落盘真源为媒体旁的 `.quapeaks` / `.mopeaks`。旧工程里已有的内联缓存继续可用，保存后自然移除。
- **波形生成内核更名为 `quapeaks`** ： 依赖和模块名同步更新；内核缺失时仍使用纯 Python 波形与 `.mopeaks` 回退，不阻断转写。
- **新增 `media_metadata.audio_track` 字段** ： 记录用户所选的当前音轨；波形缓存不再写进工程后，它是重启或换机打开工程时恢复所选音轨的唯一载体，旧工程缺失时回退为从缓存 payload 推断。

### 🐛 修复

- **波形缓存读取更稳健** ： 修复派生音频缓存签名与落点错误，并让损坏或不支持版本的缓存安全降级重建。
