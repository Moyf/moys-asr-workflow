...
- **本地对齐与时间码工具箱** ： 新增 FireRedASR2-CTC CPU 本地识别；Qwen3-ForcedAligner 与 FireRed 可作为对齐模型独立下载，其中 Qwen 对齐器复用 Qwen Local 的模型缓存；MOSS 以及 SRT / MOSP 后处理现在可以生成或补充字词级时间码。本地模型统一在「设置 → AI 模型配置」管理，ASR 与对齐模型分组显示，并只为没有原生字词时间码的 ASR 模型提供对齐模型选项。
