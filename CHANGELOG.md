...
- **官方 Windows Installer 与更新基础** ： 新增无需管理员权限的 Windows x64 Installer、稳定/Beta 频道检查、ETag 缓存、Release 清单与 SHA-256 校验；MOSE 套件与 Installer 作为官方构建单独分发，默认 GitHub Release 仍只提供公开便携包。更新过程会保留用户配置、日志和模型缓存。
