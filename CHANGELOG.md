...
- 自动后处理新增「烧录字幕」步骤：勾选后转写与后处理完成时自动把 SRT 字幕压进视频，生成新的 MP4 文件，不覆盖源视频。
- 烧录前自动把 SRT 转为 ASS 并应用「SRT 默认样式」，与手动烧录使用同一套样式配置。
- 完成后日志与结果中会显示烧录出的视频路径；批量转写的结果里同样带出 `videoPath`，可从批量列表直接打开视频。

### ✨ 提升

- Launcher「AI 模型配置」：供应商「自定义（兼容 OpenAI）」更名为「OpenAI 通用接口」；「自定义显示名称」移到 API URL 下方并标注（可选），API Key 与 API URL 增加填写提示。
- Launcher 识别方式列表重排并更新文案：阿里云百炼（QwenASR / FunASR）【推荐】、OpenAI 格式通用接口与本地模型（Beta）置顶；火山引擎（豆包）、Soniox STT（海外 / 小语种）、必剪（非官方 / 免费 / 实验性）等次要入口通过浅灰分隔线分组展示。
- 模型备注与参考价文案简化：能力一句话 + 独立一行 🪙 参考价，去掉地域分项与「需由接口返回时间戳」等冗余说明，分钟计价补充小时换算。
- DeepSeek 后处理默认模型更新为 `deepseek-flash`（旧 `deepseek-v4-flash` 已由官方停用），并支持选用 `deepseek-v4-pro`。
- Launcher 首页新增「支持 ❤️」入口，点击可查看B站小店赞助二维码；「项目官网」入口更名为「Github」。
- **烧录字幕可选视频编码器并支持自定义编码参数** ： 手动烧录新增「视频编码器」选择（自动（优先硬件）/ CPU（libx264）/ NVIDIA NVENC / AMD AMF / Intel QSV），自动模式会探测当前 FFmpeg 可用的硬件编码器并按序尝试，全部失败时回退 CPU；面板同时提供画质 CRF、编码预设和音频码率三个 FFmpeg 参数，可一键「保存为默认参数」，下次启动自动预填。画质 CRF 与音频码率对所有编码器生效，编码预设仅 CPU 编码使用。
- 烧录时可在工具箱中实时看到 FFmpeg 进度日志。
- 烧录字幕缺失文件时的提示改为明确指出是「视频文件不存在」还是「字幕文件不存在」（含路径），不再使用笼统的英文报错。
- 「停止媒体处理」按钮仅在操作进行中显示，停止时先显示「正在停止媒体处理……」，避免按钮文字闪烁。
- **关闭按钮样式统一** ： Launcher 设置弹窗与工具箱抽屉右上角的关闭按钮改为与 Editor 弹窗一致的轻量样式（去描边阴影，hover 浮现边框）。

### 🔄 变更

- 「转写后自动处理」去掉 Beta 标记；「固定处理」统一更名为「固定替换」（界面、状态提示、后端标签与文档同步更新）。
- Launcher「本地模型运行时」：运行环境目录不允许包含中文等非 ASCII 字符，填写后红字提示且不会保存（该情况下运行时会安装失败）；目录说明文案更新。
- 「压制字幕」统一更名为「烧录字幕」（工具箱按钮、标签页与文档文案），与编辑器「SRT 烧录样式」和自动后处理的「烧录字幕」步骤保持一致。

### 🐛 修复

- 修复 Premiere FCP 7 XML（实验性）导出去空隙时间线时，视频 / 音频片段的摆放位置会随空隙数量累积偏移，导致字幕越往后越提前的问题；现在媒体、音频、字幕与贴纸统一按毫秒映射后取帧，边界误差不超过 1 帧且不再累积。
- 修复点击「停止媒体处理」后偶发无响应：取消请求发生在进程启动前后的竞态窗口时，现在也能正确终止正在运行的 FFmpeg 进程。
...
速览一下，包含这些新特性：
- **ASS 字幕模式** ： 支持基础的 ASS 样式的预览和导出（并新增烧录视频功能）
- **叠加字幕轨** ： 允许两层字幕叠加并同时出现
- **说话人颜色映射** ： 可将不同颜色的字幕对应到说话人，并将名称直接写入字幕内
- **帧时间基准** ： 可以基于帧而不是毫秒来做操作
- **大量的界面重构和优化** ： 包括启动器的本地模型以及编辑器的全局设置等

### 🚀 全新特性

#### ASS 字幕模式（Beta）

![](docs/assets/1.6.0/ass-style-library.webp)

- 新增 ASS 字幕模式与独立的样式管理窗口，支持全局的渐入动画等效果。
- 支持导出标准 `.ass` 格式字幕，沿用预览的字体、字号和文字颜色。
- 增加 SRT 默认样式，将在烧录视频时采用。

#### 说话人与颜色映射

![](docs/assets/1.6.0/speaker-preview.webp)

- 新增「字幕颜色」设置页：开启「将颜色映射为说话人」后，可为每种颜色配置说话人名称，预览中显示说话人前缀，导出时也可选把说话人名称写入字幕文本（SRT 与 ASS 均支持）。
- 颜色支持自定义色值。
- 右键颜色组中的字幕可「从颜色组中脱离」，在保留当前颜色的同时不再跟随整组换色。

#### 叠加字幕轨

![](docs/assets/1.6.0/overlay-track.webp)

- 支持叠加同时出现的字幕（最多两层）
- 导入时会把双层重叠的 SRT 作为独立的叠加轨编辑，主轨与叠加轨可互相转换。
- `Ctrl/Cmd `+拖动波形或右键可创建叠加字幕；` Shift`+拖动遇到遮挡时自动转入叠加轨。

#### 帧时间基准

![帧时间基准|600](docs/assets/1.6.0/frame-timebase.webp)

*在全局设置中切换「时间基准」*

- 编辑器支持毫秒 / 帧两种时间基准，自动检测媒体 FPS 并写入工程。
- 帧模式下，拖动、边界调整、方向键等操作都以帧为最小单位，字幕自动吸附到帧。
- 波形新增支持 2 秒视窗，可以看到更大的范围，配有对应帧的网格，可精准到单帧操作。
...
- **OpenAI（及兼容接口）** ： 支持 `whisper-1 `、` gpt-4o-transcribe` 等官方模型，也可填写任意兼容 `/audio/transcriptions` 的服务。
- **OpenRouter** ： 提供可用的 OpenAI 转写模型列表与参考价格，API Key 入口直达 OpenRouter。
- **豆包（火山引擎）** ： 支持字/词级毫秒时间戳、可选说话人分离与颜色映射、即时热词直传，可通过 `VOLC_ASR_RESOURCE_ID` 切换 2.0 / 1.0 / 闲时版模型。

![](docs/assets/1.6.0/doubao-asr.webp)

#### 其他新能力

- **工程自动备份** ： 编辑器默认开启备份：自动按分钟定时备份，手动保存也会生成带时间戳的副本；可设置保留数量，最早的备份会移入回收站。

![](docs/assets/1.6.0/backup-settings.webp)

- **部分翻译回填** ： 翻译时可选「将译文回填到原字幕」——只翻译有语音的句子并直接替换原文，未翻译部分连同逐词时间码原样保留，适合只有少量外语的视频。

![](docs/assets/1.6.0/partial-translate.webp)

- **双语字幕合并** ： 可把原文与译文合并为单轨双语字幕，翻译成中文时中文自动在上；可自定义双语行序。

![](docs/assets/1.6.0/bilingual-into-one.webp)

- **视频多音轨选择** ： 多音轨视频会列出可读的音轨列表，所选音轨贯通转写、波形与导出；导出 OTIO 时每条音轨生成独立轨道，并写入达芬奇可识别的声道映射。

![](docs/assets/1.6.0/soundtrack-selection.webp)

- **本地模型扩充** ： 新增 FireRedASR2 CPU 识别模型；新增可独立下载的对齐模型（Qwen3 对齐器、FireRed CTC）与「补齐时间码」工具；Apple Silicon 上 Qwen3-ASR 自动使用 MPS 加速。
- **媒体实用工具** ： 工具箱新增「压制字幕」（SRT / ASS / SSA 烧录进视频）与「提取音频」（选择音轨输出 AAC/M4A）；Premiere XML 导出基本可用。
- **OTIO 导出选项** ： 时间线导出新增「同时导出 SRT」「包含表情包」「字幕作为片段标记」开关，选项状态会记忆。

### ✨ 提升

#### 界面大重构

![](docs/assets/1.6.0/accent-color.webp)

- **界面强调色** ： 全局设置可自定义主题色。
- **全局设置窗口** ： 「⚙️ 全局设置」改为独立可拖动的窗口，分区改为左侧标签页并记忆位置；帮助面板与工具箱同步改版。
- **整体优化** ： 简化媒体窗口与装饰细节、浮动窗口按激活顺序自动置顶、导入字幕改为预览框上方 tabs 等大量界面调整。

#### 编辑操作

- **字幕边界拖动改进** ： 两条字幕贴合时，中缝处可拖动边界让两侧联动调整；单独拖动左右手柄仍只调整当前字幕。旧的「自动吸附调整相邻字幕」移入「传统模式」开关。

![](docs/assets/1.6.0/boundary-drag.webp)

- **拆分并保留原文** ： 可把一条字幕按断点拆成两条但不切分文本，两边保留相同文本。
- **填充区间空隙** ： 波形右键新增「填充区间空隙」，把鼠标位置与前后最近的已激活空隙填充合并成一个大空隙。
- **操作与快捷键** ： 新增 `I` / `O` 跳到当前字幕开头 / 结尾；全局设置新增「播放过程中点击鼠标自动暂停」。
- **字幕列表** ： 编辑、拆分、合并及撤销重做时保持当前阅读位置；手动翻看会暂停跟随播放，可点击「跟随播放」恢复。

#### 体验细节

- **任务完成系统通知** ： Launcher 可在「设置 → 通用」开启完成通知。
- **波形振幅自动缩放** ： 打开工程时按媒体响度自动选择波形的垂直缩放。
- **文稿匹配改进** ： 新增预览与错误提示，预览、手动匹配与自动后处理共用断句、标点与清理设置；明显属于文稿之外的录音段会被保留，不再被吞入相邻句。
- **服务器编辑器** ： 打开工程时显示加载进度；服务器断开时自动提示并可重启，重启后不再重复打开页面导致内容丢失。
- **Launcher 错误提示** ： 常见错误提供中英文操作建议。
- **本地运行环境安装更稳** ： 安装失败或取消后自动恢复为可重试状态；提前检查 PyTorch 下载源，支持 `MAW_PYTORCH_INDEX` 切换镜像，减少国内网络的长时间等待。
- **启动与加载更快** ： Launcher 首屏先显示再加载后台检查，减少启动白屏与等待。
- **实用小工具** ： 新增 `tools/compare.html`（转写结果对比）与 `tools/timestamp-compare.html`（字词时间码查看 / 对比）。

### 🔄 变更

- **波形缓存与工程体积** ： 波形缓存改为二进制的 `<媒体>.quapeaks` / `.mopeaks `，` .mosp` 工程不再内联波形数据，体积不再随素材时长暴涨；升级后首次打开会重新提取一次波形。

> 感谢 @量子猫 的 quapeaks 支持！

- **输出文件归入 `_maw`** ： Launcher 默认把输出文件放入媒体旁的 `_maw` 子文件夹，并默认保留后处理的中间产物（均可在设置中关闭）；产物与导出文件名的操作后缀在中文界面下本地化（如 `去空隙 `、` 表情包 `、` 红色`）。
- **语言与地域设置** ： 界面语言移入「⚙️ 设置 → 通用」，首次使用按系统语言自动选择；Qwen 阿里云百炼地域与 Workspace ID 可直接在设置中配置，海外地域不再需要手动编辑 `.env`。
- **额外断句符号** ： 「设置 → 断句与标点」的额外断句符号现在对云端转写同样生效，在这些符号处强制断句。
- **转写 CLI** ： 新增 `--no-model-tag`；转写完成输出耗时、媒体时长与实时率，附带费用估算。
- **本地运行环境** ： local 与 OCR 运行环境分开管理、独立安装；可在「设置 → 运行环境」自定义安装目录。
- **设置项整理** ： 切句参数移入「设置 → 断句与标点」；「大语言模型（AI）」更名为「AI 模型配置」。

### 🐛 修复

- **超长字幕不再漏切** ： 云端返回缺少词级时间码、MOSS 等段级模型、本地 Qwen 对齐退化时，此前会输出几十字不切分的超长字幕；现在会保留句边界并对超长段二次拆分。中英文可分别配置最大字数 / 单词数与合并阈值。
- **文稿匹配** ： 问号等断句符号后的 `」 `、` ” `、` 》` 等闭合标点不再被误删。
- **拆分手动改稿字幕** ： 文本被人工改过、与词时间戳脱钩时改用顺序保持的对齐算法，拆分不再错位。
- **波形与边界** ： 修复部分采样率下波形与频谱随播放逐渐错位、立体声只显示第一声道、「按音量移除空隙」未使用当前波形，以及超过 2GB 的长视频生成波形失败的问题。
- **编辑器稳定性** ： 打开长媒体大工程时不再误报「编辑器服务器没有响应」；LLM 流式响应在服务未声明字符集时中文不再乱码；翻译漏答时自动补救，失败后保留原始转写并可从失败步骤重试。
- **工程兼容性** ： 工程跨系统（Windows ↔ macOS / Linux）移动或与媒体一起改名后仍可定位媒体；转写来源、切句方式等工程元数据在保存往返中不再丢失；修复颜色组拆分后悬空引用导致工程加载失败的问题。
- **本地模型** ： 修复有 NVIDIA 显卡的电脑安装 / 修复本地运行环境时因缺少 `sherpa-onnx-core` 依赖而失败的问题；修复 MOSS 等托管环境未装波形内核时启动即失败的问题。
- **其他** ： macOS Dock 图标显示正确；OCR 在拖入新视频后自动跟随；英文界面补齐配置文案与缺失词条。

## [1.6.0-beta.6] - 2026-09-20

### ✨ 提升

- **ASS 预览换行**：ASS 字幕模式不再因播放器预览框宽度自动折行，只保留字幕文本中的显式换行；ASS 导出格式保持不变。
- **ASS 说话人名称颜色**：新增仅为 `XX：` 说话人前缀应用颜色的 ASS 预览与导出模式，正文继续使用当前 ASS 样式的主色。
- **拆分面板**：新增「拆分并保留原文」，可在不切分字幕文本的情况下把一条字幕按当前断点拆成两条，并让两边保留相同文本。
- **说话人预览与导出联动**：颜色映射开启时默认显示预览中的说话人名称，并自动勾选导出名称；ASS 颜色字幕选择「作为说话人名称颜色」时提供跳转导出设置的提示。

### 🔄 变更

- **输出目录与后处理产物默认值**：Launcher 默认将所有输出文件放入媒体旁的 `_maw` 子文件夹，并默认保留自动后处理的中间产物；运行目录额外保存编号为 0 的原始快照，后续 `.mosp` / `.srt` 阶段按序号和阶段名命名；两项仍可在设置中关闭。

### 🐛 修复

- **文稿匹配** ： 保留录音中明显属于文稿之外的 ASR 字幕段；重新分段时不会再把中间多录的句子吞入后一个文稿句。

- **双语字幕中缝位置** ： 修复中缝联动区错误落在副字幕 lane、导致主字幕中缝无法正常命中的问题。

### 🐛 修复

- **文稿匹配的闭合符号**：修复问号等断句符号后的 `」 `、` ” `、` 》` 等常见闭合标点在部分工程中被误删的问题。
- **播放点击暂停**：全局设置新增「播放过程中点击鼠标自动暂停」，开启后播放中通过鼠标点击字幕、波形或空隙跳转会在定位后暂停，默认关闭。

## [1.6.0-beta.5] - 2026-09-20

### ✨ 提升

- **Premiere XML**：改进导出格式，目前在 PR 内基本可用。
- **本地识别与对齐**：新增 FireRedASR2 CPU 识别模型；可独立下载两款对齐模型用于生成时间码（Qwen3 对齐器、FireRed CTC）；同步增加「补齐时间码」工具箱。
- **调试产物**：本地模型也支持「调试运行」并生成阶段文件。
- **Launcher**：编辑器服务器断线重启后，工程未变则不重复打开新页面，避免内容丢失。
- **叠加字幕轨** ： 支持双层重叠 SRT 作为独立轨编辑；主轨与叠加轨可互转，第三层报错； `Ctrl/Cmd` +拖动波形或右键可创建叠加字幕， `Shift` +拖动遇遮挡自动转入叠加轨。
- **ASS 字幕模式** ： 新增默认关闭的 ASS 模式与独立样式管理窗口，支持全局的渐入动画等效果。

### 🎨 界面

- **Launcher UI 更新**：改进了运行时和本地模型的配置页面。
- **断句设置**：按「停顿切句 → 短句合并阈值 → 最大字数」排列，合并前先检查最大字数。
- **波形与设置**：波形面板的操作类设置移入「全局设置 → 通用操作」。
- **导入字幕弹窗**：「替换当前字幕 / 作为副字幕」改为预览框上方 tabs，按钮居中。
- **动态图形合成尺寸**：Lottie / OGraf 导出弹窗打开时自动匹配媒体宽高，增加自定义宽高设置。

### 🪒 工具

- **转写对比工具 `tools/compare.html`**：用于对比多模型生成的 SRT/`.mosp` 字幕。
- **时间码工具 `tools/timestamp-compare.html`**：查看字词时间码的工具，同样可以用于对比。

### 🐛 修复

- **传统模式边界光标**：选「传统模式」时字幕边界手柄恢复系统左右箭头光标，原创光标只用于中缝联动模式。
- **浮层与弹窗层级**：右键菜单提升到浮层之上，修复 ASS 样式库列表右键无菜单；模态弹窗遮罩可覆盖浮层与顶栏，弹窗不再被挡。
- **拆分手动改稿字幕不再错位**：文本被人工改过导致与词时间戳脱钩时，改用顺序保持的对齐算法；全对不上时按文字位置映射回退。

## [1.6.0-beta.4] - 2026-09-15

### ✨ 提升

- **颜色组字幕脱离** ： 右键颜色组中的字幕 cue 可在保留当前颜色的同时从颜色组中脱离；原组其余字幕会自动维持有效分组，单独着色的字幕不显示该操作。
- **字幕导出菜单整理** ： 导出字幕菜单的布局优化；增加 ASS 按空隙导出的功能。
- **任务完成系统通知** ： Launcher「⚙️ 设置 → 通用 → 完成通知」可以启用系统通知。
- **波形振幅按媒体响度自动缩放** ： 编辑器首次打开工程时，会用波形响度自动为波形选定垂直缩放。
- **Apple Silicon 本地识别加速** ： Qwen3-ASR 的自动设备模式在支持时使用 MPS；初始化失败时自动回退 CPU。
- **自定义合并双语字幕的顺序**： 勾选合并双语字幕时可选「双语行序」下拉列表。
- **翻译回填与目标语言处理** ： 翻译任务新增与合并双语互斥的「将译文回填到原字幕」输出方式：适用于仅有少量语音需要翻译的情况，翻译结果直接替换原字幕对应句子，只输出单条字幕，未翻译的句子连同逐词时间码原样保留。

![partial-translate](docs/assets/1.6.0/partial-translate.webp)

- **字幕边界拖动改进**： 两条字幕贴合时，中缝处新增可拖动的边界区，拖动中缝两侧边界一起联动；单独拖动左右手柄仍只调整当前字幕（类似达芬奇的剪辑逻辑）。旧的「自动吸附调整相邻字幕」开关移入「传统模式」，可在「音频波形区 ⚙️ → 操作 → 贴合字幕边界」中切换。

![boundary-drag](docs/assets/1.6.0/boundary-drag.webp)

- **Launcher 错误诊断与脱敏** ： 常见后处理、批量任务、MOSE 和口播对齐错误现在提供中英文操作提示；字幕编辑服务器断开时会自动提示并可重新启动。

### 🐛 修复

- **本地转写断句设置对 MOSS 等段级输出生效** ： MOSS、FunASR / SenseVoice 及 Qwen3-ASR 在模型未返回词级时间戳时，超长段不再整段保留为一条字幕：中文按「最大字数 / 短句合并阈值 / 停顿切句」在标点处二次拆分，英文按「最大单词数 / 短句合并阈值」在单词边界拆分；段内时间为插值近似值，段首尾保持模型真实时间。
- **颜色分组批量换色** ： 修复拆分后的部分颜色组批量换色时遗留无效引用，工程不再因悬空 `headIdx` 加载失败。
- **彩色字幕列表序号可读性** ： 提高有颜色字幕行中 idx 序号的对比度，避免序号被彩色背景压低后难以辨认。
- **ASS 说话人名称导出** ： 「导出时附加说话人名称」现在同时作用于 SRT 和 ASS 导出。
- **本地运行环境安装更稳** ： 安装失败、取消或异常退出后，Launcher 会自动恢复为可重试状态；安装时会提前检查 PyTorch 下载源，并支持通过 `MAW_PYTORCH_INDEX` 切换到可达镜像，减少国内网络环境下的长时间等待与模糊报错。
- **Linux 打包依赖更稳定** ： AppImage 打包使用 stable 版本的 FFMpeg 版本。
...
- **自动后处理翻译配置提示** ： 设置页的「大语言模型（AI）」更名为「AI 模型配置」；从「后处理步骤」进入尚未验证的 LLM 配置时，「测试连接」按钮会持续黄色闪烁提醒。
- **豆包 API Key 入口** ： Launcher、CLI 提示和工作流文档统一改用火山引擎新版 API Key 管理页。
- **Launcher 默认输出命名** ： 关闭「在输出文件名中附加模型名称」时，必剪字幕默认保持为 `<媒体名>.srt`；开启后仍使用 `.bcut.srt` 后缀区分来源。

- **Launcher 设置入口与地域配置** ： 界面语言从页首移入「⚙️ 设置 → 通用 → 语言」；首次使用会按系统语言自动选择中文或英文，手动切换后立即保存。Qwen 的阿里云百炼地域和 Workspace ID 现可在「⚙️ 设置 → AI 模型配置」底部配置，高级选项提供直接跳转，海外地域不再需要手动编辑 `.env`。

### 🔄 变更

- **Launcher 界面调整** ： 暂时移除未开放的「时长上限」输入框（底层时长限制参数仍保留）；在「项目官网」旁增加「教程视频」链接，直达 B 站教程视频。
- **阿里云百炼 API Key 入口** ： Launcher、CLI 提示和工作流文档改用 [千问平台](https://platform.qianwenai.com/home/)，和云百炼互通，但页面设计更佳。
- **Launcher 文件输出与表情包路径** ：「在输出文件名中附加模型名称」默认关闭；「默认表情包路径」的当前值会在目录有效时显示为可点击路径，点击打开文件夹，失效路径不会调用系统打开。
...
![doubao-asr](docs/assets/1.6.0/doubao-asr.webp)

- **豆包语音识别供应商（火山引擎）** ： 新增豆包大模型录音文件识别作为云端转写供应商。支持字/词级毫秒时间戳、可选说话人分离与颜色映射、即时热词直传，并可通过 `VOLC_ASR_RESOURCE_ID` 切换 2.0（默认 Seed-ASR）/ 1.0 / 闲时版模型。
- **OpenRouter OpenAI 兼容 ASR** ： Launcher 新增 OpenRouter 可用的 OpenAI 转写模型与参考价格；使用 OpenRouter 时，内置模型会自动补全 `openai/` 模型前缀，并在 API Key 入口提供 OpenRouter。其他云端 API 模型也会显示服务商参考价格。
- **OpenAI 转写高级设置** ： 按模型显示并转发 Prompt、Keywords；新增官方 `gpt-transcribe` 和 `gpt-4o-transcribe-diarize` 选项。说话人模型会自动请求 `diarized_json` 并保留 speaker 标签。

#### 界面大重构

![accent-color](docs/assets/1.6.0/accent-color.webp)

- **界面强调色** ： 全局设置「基础 → 界面 → 外观」新增主题色的设置；选中的颜色圆点改用白色填充，避免与选项背景融为一体。
- **整体 UI 优化** ： 默认隐藏分隔条，简化媒体窗口显示，去掉冗余装饰，etc。
- **「视频预览」与「字幕样式」设置分区** ： 全局设置的「预览字幕」标签页升级为「视频预览」，大量重构设置页面。
- **浮动窗口层级** ： 全局设置、帮助等窗口按最后激活顺序自动置顶，避免被先打开的浮层遮挡。

#### 说话人与颜色映射

![speaker-preview](docs/assets/1.6.0/speaker-preview.webp)

- **字幕预览说话人标签与颜色样式** ： 新增独立的「字幕颜色」设置页；开启「将颜色映射为说话人」后，可配置各字幕颜色对应的说话人名称（默认 `SP1 `～` SP5`）；导出字幕时可选添加说话人到字幕文本中。

### ✨ 提升

![backup-settings](docs/assets/1.6.0/backup-settings.webp)

- **工程版本备份** ： Editor「保存」设置新增默认开启的工程备份，本地 Server 绑定工程可按分钟定时备份，手动保存也创建带时间戳的 `.mosp-bak`；备份放入工程对应的 `_maw/备份`（英文界面为 `backups`，切换界面语言前的旧目录仍可打开），可设置最多保留的版本数，超出后将最早版本移入回收站。

- **主副字幕交换的颜色映射** ： 交换时按绑定关系将主字幕颜色映射到新的主轨，并保留移到副字幕的原主轨 `color` / `color_ref` 信息。
- **分色导出文件名中文化** ： 编辑器界面语言为中文时，按颜色导出的 SRT 文件名颜色段同步使用中文（如 `clip_红色.srt `、` clip_去空隙_蓝色.srt `），未标记颜色的默认组后缀为「默认」（` clip_默认.srt`）；英文界面保持原样。
- **服务器工程启动提示** ： 本地 Server 编辑器在后台读取工程、准备媒体和生成波形期间，首屏直接显示「正在加载工程…」及进度状态，并完善加载进度条的显示。
- **口播对齐读取媒体波形缓存** ： 口播对齐启动时会按工程所选音轨读取媒体旁或 `_maw` 中的 `.quapeaks` wave 层，供自动静音空隙检测使用；缓存只在运行态使用，不会写回导出的工程。

- **额外断句符号作用于云端转写** ： 「设置 → 断句与标点」的「额外断句符号」现在同时作为云端转写（Qwen / Qwen-Audio / Fun-ASR）切句时的强断句符号，在这些符号处强制断句；此前该配置仅对文稿匹配后处理生效，中文界面与英文界面说明已同步更新。
- **文稿匹配预览与错误提示** ： 文稿匹配新增默认开启的 Markdown 符号清理，并让文稿预览、拆分预览、手动匹配与自动后处理共用断句、句尾标点和清理设置；默认不再把逗号、句号写入匹配结果。匹配产物按界面语言使用 `文稿匹配` / `match` 后缀；非法字幕/工程解析、文稿解析和低匹配度会分别提示并在重新计算后刷新预览。
- **后处理产物命名本地化补全与界面提示** ： 中文界面下所有后处理产物的操作后缀统一本地化（`校对文本` / `重新断句` / `自定义`，此前保持英文原名；OCR 报告同步使用 `OCR去重` 后缀），旧文件名仍可正常识别；固定处理的产物后缀按实际启用的部分细分（只有批量替换为 `.批量替换`，只有简繁转换为 `.转简体` / `.转繁体`，两者都有时组合为 `.批量替换.转繁体`），勾选了固定处理但既无规则也未选转换方向时跳过该步骤、不产出文件；自动后处理成功且未保留中间产物时，清空后的「后处理」目录会一并移除，不再留下空文件夹；自动后处理链中的「LLM 校对」「重新断句」以灰色斜体显示并附悬浮说明，工具箱 LLM 任务的默认顺序调整为「翻译成中文 → 翻译成英文 → 校对文本 → 重新断句 → 自定义」。

### 🔄 变更

- **波形缓存改为二进制容器** ： 转写生成的缓存改为 `<媒体>.quapeaks`，编辑器按需提取的缓存改为 `<媒体>.mopeaks`；旧的 `waveform.json` 不再生成或读取，升级后首次打开会重新提取一次波形。REAPER 原生 `.ReaPeaks` 继续照读。
- **波形数据不再内联进工程文件** ： `.mosp` / 工程 `.json` 只保存字幕、设置与当前所选音轨，工程体积不再随素材时长暴涨。
- **波形生成内核更名为 `quapeaks`** ： 依赖和模块名同步更新；内核缺失时仍使用纯 Python 波形与 `.mopeaks` 回退，不阻断转写。

> 感谢 @量子猫 的 quapeaks 支持！

- **拆分副字幕入口随多重字幕开关** ： 波形空白处右键菜单中的「按音频位置拆分副字幕」仅在开启多重字幕时显示；关闭多重字幕（副轨数据仍保留）后不再出现该入口。
- **多重字幕下隐藏「仅看超长」** ： 「仅看超长」按单轨文本字数筛选，多重字幕双列下主/副两栏合并计数无意义，故隐藏。

### 🐛 修复

- **云端转写降级不再放弃断句** ： 当云端返回中部分句子缺少可用的词级时间码（时间码缺失、倒挂或与句文本不匹配）时，此前整份结果会跳过全部切句逻辑，输出大量几十字、带逗号却不拆分的超长字幕；现在改为保留服务端句边界并对超长段二次拆分——有词级时间码的段仍按词级精确拆分，没有的段按标点切块并在句段范围内按字数占比插值时间，确保拆句。
- **本机 Server 新手引导状态** ： 「快速上手」的完成 / 跳过状态现在保存在用户级设置中，换用新的 localhost 端口后不会重复弹出；自包含的 `file://` 编辑器仍使用浏览器本地存储。
- **本地 Qwen 字幕分段** ： 修复词级对齐中个别零时长词导致整个识别块退化为约 30 秒一条字幕的问题；保留完整文本与其余有效切句边界，对无法恢复的对齐结果增加日志提示。
- **工程媒体跨系统定位** ： 工程从 Windows 移到 macOS/Linux，或同目录工程与媒体一起改名后，编辑器可继续定位媒体，并将失效引用修复为便携的同目录相对路径。

## [1.6.0-beta.2] - 2026-09-07

### ✨ 提升

- **填充区间空隙** ： 波形右键菜单在分隔线下方新增「填充区间空隙」（空隙操作移至菜单底部，仅在已有已激活空隙时显示），把鼠标位置与其前后最近的已激活空隙完全填充并合并成一个大空隙，中间的未激活空隙会被直接吞掉；鼠标位于所有已激活空隙之前/之后时向开头/结尾拓展。
- **全局设置窗口** ： 编辑器「⚙️ 全局设置」从上方展开面板改为独立的可拖动设置窗口（Esc、关闭按钮或再次点击按钮关闭，位置自动记忆），通用操作、时间基准、拆分与合并、导出、保存、表情包、彩蛋七个分区放入窗口左侧的垂直标签页，并记忆上次停留的分区；时间基准内各设置与说明改为分组配对展示。
- **帮助面板** ： 「🤔 帮助」窗口沿用全局设置窗口的视觉风格：顶部标签换成左侧垂直标签页，进阶分类（微调字幕 / 空隙操作 / 批量操作）平铺展示、不再需要展开，面板与头部的渐变背景改为纯色卡片；快捷键黄色高亮、窗口拖动与右下角缩放保持不变。
- **工具箱实用工具布局** ： 「实用工具」改为左侧垂直标签页，工具面板可以独立切换，减少工具数量增加后的横向拥挤；标签列与工具面板现在各自独立垂直滚动，标签列略微收窄且不再为滚动条保留占位空间。
- **Launcher 切句设置** ： 将最大字数 / 单词数、合并阈值和停顿切句从高级设置移到「设置 → 断句与标点」，校验失败时自动定位到对应设置页。
- **本地模型运行时入口** ： 将本地模型运行时的安装与修复移到「设置 → 运行环境」（位于 OCR 支持上方）；本地模型面板上方保留运行环境检测结果，并提供「在 ⚙️ 设置中查看」链接直接跳转（深度链接直接落在 Runtime 标签页顶部）；「本地运行环境已就绪。」等后端固定提示在英文界面下改为本地化文案；运行时 beta 说明与供应商风险提示共用琥珀 callout 样式。
- **本地运行环境自定义目录** ： 与 OCR 一致，本地模型运行环境可在「设置 → 运行环境」中自定义安装目录（写入 `MAW_LOCAL_RUNTIME_ROOT`，即时生效并持久化）；也支持把文件夹拖入输入框；「模型缓存：…」链接行移到本地模型面板「模型保存目录」说明下方。
- **英文界面配置文案** ： 识别方式 / 模型下拉选项、模型与供应商说明、语言与地域选项等由后端配置下发的中文文案，在英文界面按稳定 ID 映射为英文（如「本地运行；首次准备会加载 Qwen3-ASR 与 Forced Aligner」）。
- **Launcher 视觉微调** ： 去除页首横幅与本地模型面板的渐变底色；设置弹窗加宽至 720px 并设定最小高度；设置内容区滚动时顶 / 底部增加与工具箱一致的渐隐过渡（scroll-driven 遮罩），不再生硬截断。
- **时间线 OTIO 导出选项** ： 「时间线 OTIO 工程 / OTIOZ 打包工程」（含去空隙版本）的菜单下方新增三个默认勾选的开关：同时导出 SRT 字幕、时间线包含表情包（作为叠加视频轨合并进同一个 OTIO 工程，OTIOZ 会把图片一并打包）、将字幕内容作为标记写入片段；选项状态跨菜单同步并记忆。「去空隙版本」与「更多导出」菜单取消了分组分隔线，由二级子菜单承担分组。

### 🐛 修复

- **Launcher 缩放后的操作栏定位** ： 修复缩放 Launcher 后底部操作栏随页面上移的问题；操作栏现在始终贴齐可见窗口底部。
- **MOSP 工程版本与元数据保真** ： 工程生成、服务器保存和浏览器导出统一写入 `moy.asr.project.v1`；旧工程保存时自动补写版本，未知新版本会在覆盖当前工程前被拒绝，编辑器往返保存不再丢失转写来源、切句方式、时间码粒度及未知可选顶层字段。
- **OTIO 空媒体元数据导出** ： 修复旧工程或元数据缺失的工程在导出 OTIO 时因音轨占位值解构失败而中断的问题。
...
- **macOS Dock 图标** ： Launcher 启动时改用 App 内的 ICNS 图标，避免 macOS Dock 显示 Windows 方形图标。
...
- **转写 CLI 输出命名与耗时日志** ： 六个转写 CLI 的默认文件名不再附带倍速段，新增 `--no-model-tag`（省略模型/供应商/引擎标识段）开关；转写开始与结束输出时间码，完成后输出转写耗时、媒体时长和实时率说明（转写时长为媒体时长的倍数）；使用阿里云百炼服务时按 `0.00022 元/秒` 附带费用估算；转写结束后在 stdout 输出机器可读的 `MAW_STAT rtf=0.123` 行；`--debug-raw` 的 `asr-response.json` 在未指定 `--output` 时改写入媒体旁的 `_maw` 目录。
- **Launcher 文件输出设置** ： Launcher「通用」新增「文件输出」分组，可把全部输出放入媒体旁的 `_maw`（或每个视频独立的 `视频名_maw`）子目录、按需省略 SRT 文件名中的模型/供应商段；编辑器 HTML 与批量清单等辅助文件默认归入 `_maw`，最终字幕默认留在媒体旁。
- **后处理耗时统计** ： 自动后处理记录每个步骤的耗时并在 manifest 中保存 `elapsedSeconds`，完成后输出后处理总用时；Launcher 开启自动后处理时汇总「转写 + 后处理」的全程总用时、各段耗时和全程相对媒体时长的倍数。
- **产物后缀本地化** ： 文件名操作后缀跟随 Launcher 界面语言：中文界面下后处理产物为 `.后处理`、OCR 去重为 `.OCR去重`、翻译段为 `.翻译为中文` / `.翻译为英文`、双语标记为 `.双语合一`；英文界面保持 `.postprocess` / `.ocr-dedup` / `.translate-*` / `.bilingual` 原样，旧英文命名文件的反查与重复翻译防护不受影响。后处理中间产物目录从 `MAW-Postprocess` 改为媒体旁 `_maw` 内的 `后处理`（中文界面）或 `postprocess`（英文界面）。
- **编辑器导出与媒体工具命名本地化** ： 编辑器界面语言为中文时，导出文件名中的 `gap-removed `、` stickers` 段显示为 `去空隙 `、` 表情包`（如 `clip_去空隙.srt `、` clip_去空隙-表情包.otioz`）；媒体重组、压制字幕与提取音频的产物后缀同步本地化为 `去空隙` / `压字幕` / `音频`，媒体反查兼容新旧两种命名；英文界面保持原样。
