...
- **动态图形合成尺寸自动匹配与自定义** ： Lottie / OGraf 动态字幕导出弹窗的「合成尺寸」在打开时自动匹配工程媒体元数据的宽高（命中横屏 / 竖屏 / 4K 预设时直接选中），不再需要手动核对；媒体为不规则尺寸时自动落到新增的「自定义…」选项并预填媒体宽高，也可以手动填写 16–7680 范围内的任意整数宽高。
- **Premiere 交接 XML 默认写入原生字幕文本** ： 「Premiere FCP 7 XML（实验性）」导出弹窗中「写入原生字幕文本对象」移到「导出字幕轨」之前并默认勾选；未勾选原生文本时「导出字幕轨」选择框置灰禁用——该选项只在写入原生文本时参与导出，之前不勾选时选了也不生效，容易造成可用的假象。
- **转写对比小工具** ： 新增 `tools/compare.html`，浏览器直接打开即可使用：把多个模型生成的 SRT 或 `.mosp` 工程拖进页面，按时间对齐成组并排查看各模型的转写文本；可将任一文件设为「基准」，查看逐句字符相似度、相对基准的差异高亮与整篇相似度评分，并支持「只看差异」和文本搜索。文件名共享的视频名前缀会自动省略，页面顶部显示「视频名」的字幕对比（N 个模型）；底部「时间分布」视图复刻波形区多层字幕轨外观，按文件分轨展示时间分布，可拖动调整高度、水平 / 垂直缩放（Ctrl/Alt+滚轮）。
- **时间码查看与对比工具** ： 新增 `tools/timestamp-compare.html`，支持单个 `.mosp` / `.json` 工程查看段级与字词时间码，也支持与第二个工程按段序号对比起止时间、字词数量、文本和逐项偏差；SRT 可作为无字词时间码的段级参照，点击行可展开明细，顶部开关可在色块上方单独一行显示较大的 item 文本，并以奇偶错色区分时间块；段和 item 都显示两位小数 duration，越界 item 使用红色、空 items 使用独立样式，并可按空 items 或越界项筛选；表格对大量段启用可见行懒渲染，调整窗口时减少标签重排；支持嵌入工程数据自动生成报告页面。
- **本地对齐与时间码工具箱** ： 新增 FireRedASR2 CPU 本地识别，默认串联 FireRedASR2-CTC 与现有本地运行环境中的 FunASR `ct-punc`，按标点重新断句并保留原始字词时间码；也可在「高级选项」中关闭 ct-punc，直接使用 CTC 结果。Qwen3-ForcedAligner 与 FireRed CTC 可作为对齐模型独立下载，其中 Qwen 对齐器复用 Qwen Local 的模型缓存；MOSS 以及 SRT / MOSP 后处理现在可以生成或补充字词级时间码。本地模型统一在「设置 → AI 模型配置」管理，ASR 与对齐模型分组显示，并只为没有原生字词时间码的 ASR 模型提供对齐模型选项；识别时的对齐模型选择也收进「高级选项」；本地 ASR 分组列出全部可用本地模型及其准备状态，可直接切换查看；本地模型运行时设置增加可展开的运行时清单，可查看版本、Python 版本和依赖安装情况，运行环境安装或修复完成后清单自动刷新；运行环境升级到 v7（新增 sherpa-onnx 与 soundfile），已安装的旧环境需按提示重装一次。
- **本地调试产物** ： 「调试运行」按识别来源路由：在线模型继续保存服务端原始 JSON；本地模型生成统一调试清单和阶段文件。FireRed 额外保存 CTC 原始字词时间码、ct-punc 标点结果与最终断句结果，便于定位识别、标点或分句阶段的问题；启用输出子文件夹时，在线与本地调试文件统一进入对应 `_maw/调试`（英文界面为 `_maw/debug`），并遵循每视频独立子文件夹设置。
- **Launcher 断句设置顺序与合并约束** ： 设置页按「停顿切句 → 短句合并阈值 → 最大字数」排列；短句合并前会检查最大字数，避免合并后再拆分造成不必要的断句。
- **运行环境本地模型入口提示** ： 识别方式未选「本地模型」时，「⚙️ 设置 → 运行环境」顶部会显示「本地模型」提示，说明需要先在「识别设置」中切换识别方式才能查看本地模型相关配置。
- **Launcher 断线重启不再重复打开页面** ： 字幕编辑服务器意外断开后，点击「启动字幕编辑器」重启成功且工程未变时，不再重复打开新的编辑器页面，只更新状态提示；回到原页面（必要时刷新）即可继续编辑，避免编辑到一半的内容因换页而丢失。
- **波形设置精简与行高拓展** ： 波形区 ⚙️ 面板中的操作类设置移入「全局设置 → 通用操作」；双语字幕设置中的补上新增的高档位。
- **ASS 样式库样式方案表单重构** ： ASS 输出方案表单中「使用样式」改为「主字幕样式」，开启多重字幕时右侧显示「副字幕默认样式」（即原默认槽位中的副字幕样式，不再单独配置）；`\fade`、`\move`、`\t` 收进「进阶动画」折叠分组，勾选框直接显示在名称旁（去掉「启用」文字），α 与 t 参数分行排列，`\t` 的「ASS 标签」更名「自定义 ASS 标签」并加大输入框、启用时以强调色描边提示；「当前方案」摘要卡背景跟随明暗模式，修复亮色模式下文字不可读。样式库中的辅助文字统一提升到 12px，列表项按压不再挤出滚动条，对齐九宫格放大并调整表单布局，字体旁按钮改为「读取本机字体」并与设置页共用字体扫描。
- **导入字幕弹窗布局** ： 「替换当前字幕 / 作为副字幕」（原「作为双语字幕」）改为预览框上方的左侧 tabs，「取消 / 导入」按钮居中。
- **重叠 SRT 字幕轨** ： 支持双层重叠 SRT 导入为「叠加字幕」独立轨，与主字幕时间重叠、并排编辑，可标记颜色/表情包、拆分合并、禁用、撤销重做与播放预览，并随字幕（SRT/ASS/分色）、FCP7 XML 与 OTIO 导出；主轨与叠加轨可互相转换（颜色/表情包标记跟随）；第三层重叠会明确报错。叠加轨字幕行的序号徽标在英文界面显示为 `OVL` 前缀（如 `OVL1`）。启用「叠加字幕」轨后，在已有主字幕的位置按住 `Ctrl/Cmd` 拖动波形（或空白波形右键「创建叠加字幕」）可直接在叠加轨创建与主字幕重叠的新字幕；`Shift`+拖动主字幕遇到邻居遮挡时自动转入叠加轨继续移动，拖回无重叠位置自动放回主轨。ASS 导出中叠加字幕以独立层渲染在主字幕上方，主字幕底边距相应由 40px 调整为 80px。叠加字幕的点击行为与主字幕对齐：字幕列表与波形字幕块的普通点击遵循「点击字幕块时」全局设置（仅选中 / 选中并跳转 / 选中并跳转自动播放）；点击叠加字幕会像点击副字幕一样清空主轨/副轨已有选区；`Ctrl/Cmd`+点击切换本轨多选、`Shift`+点击范围多选，列表行、波形块与选中计数同步刷新（此前波形上 Ctrl+点击叠加字幕会误选主字幕，列表中叠加行选中也没有任何视觉反馈）。
- **ASS 样式库与预览模式** ： Editor 新增默认关闭的「ASS 字幕模式」和独立样式管理窗口，支持用户级样式、ASS 输出方案、默认槽位，以及颜色、字体、粗体、斜体、下划线、描边、阴影、对齐等 ASS 样式；ASS 导出读取默认 ASS 方案，并可逐句添加 `\fad`、`\fade`、`\move`、`\t`，Launcher 压制 SRT 读取 SRT 默认样式。localhost Editor 与 Launcher 共享本机用户级配置，便携 `file://` Editor 使用浏览器本地副本。字体设置改为文本输入 + 可筛选下拉列表（按显示名去重、前缀优先筛选，输入框与列表同宽紧贴，支持本机字体扫描和方向键 / Enter 选择）；启用「ASS 字幕模式」后「预览字幕颜色」由 ASS 接管并给出跳转提示；默认 ASS 样式按操作系统选择中文字体（Windows 微软雅黑 / macOS 苹方 / 其他 Noto Sans CJK SC）并默认加粗；内置「ASS」样式与方案更名为「ASS 默认样式」「ASS 输出方案」，用户自定义过的名称不受影响。ASS 预览在小窗、全屏和舞台尺寸变化时按 PlayRes 重新计算，主字幕与说话人标签保持相同的字体和样式。多重字幕的副字幕随 ASS 导出与预览：所有副字幕共用一个「ASS 副字幕样式」内置槽位（可在样式库中改选任意样式，边距、对齐、字号完全由该样式决定），ASS 字幕模式下「字幕样式」设置的主/副字幕 CSS 控件会替换为样式库选择器并与样式库同步；多重字幕关闭时副字幕不再随 ASS 预览与导出，去空隙 ASS 导出也包含副字幕与叠加轨。与「叠加字幕」轨共用时，叠加字幕导出为独立的 Overlay 样式（字段与主字幕一致，锚定边距固化为「下方最近一层字幕的边距 + 1.2 倍字号」，在两层文字间留出空档；有副字幕时自动叠在副字幕上方，并沿用其锚定层的对齐与水平边距），颜色映射与主字幕一致，并逐句应用 `\fad`、`\fade`、`\t`（`\move` 的绝对坐标只属于主字幕）。样式库的删除操作跟随当前表单标题，SRT 默认样式表单补充烧录说明，默认槽位选择器补充用途提示，ASS 样式表单显示 ASS 模式启用状态，左侧列表保持简洁。

### 🐛 修复

- **传统模式边界光标** ： 「贴合字幕边界」选择「传统模式」时，字幕边界手柄恢复旧版的系统左右箭头光标；原创光标素材仅在中缝联动（新）模式下使用。
- **Premiere 交接 XML 序列与原生文字** ： 导出的 FCP 7 XML 现在随工程声明序列的帧率与视频宽高（取自工程媒体元数据，缺失时回退 1920×1080），Premiere 导入时按声明创建序列，不再回退到默认的 DV NTSC 720×480 小画面；原生 GraphicAndType 文字按 Premiere 原生结构补齐变换参数、Graphic 画布尺寸与 Vector Motion 组，导入后直接可见且落在「水平居中、偏下」的位置，文字按字幕内容命名并默认段落居中；字号锚定 4K 序列的 120 并随序列分辨率等比缩放，跨分辨率视觉大小一致；文字字体优先沿用「预览字幕」选择的字体，未选择时落到系统中文字体（Windows 微软雅黑 / macOS 苹方），不再使用 Arial。
- **本地运行环境依赖检查** ： 修复 `soundfile` 以顶层模块文件安装时被误判为缺失的问题，避免运行环境安装完成后仍反复显示「修复运行环境」。
- **本地转写原生崩溃给出可操作提示** ： 本地转写子进程以 Windows 原生崩溃退出码（如 0xC0000005 访问冲突，常见于显卡驱动 CUDA 初始化失败）结束时，错误信息会附带排查指引：重启电脑、更新 NVIDIA 驱动或改用 CPU 设备重试。
- **编辑器浮层与弹窗层级整理** ： 右键菜单提升到浮层窗口之上，修复在 ASS 样式库列表上右键看不到菜单的问题；导入字幕等模态弹窗的黑色遮罩现在能盖住全局设置等浮层窗口和顶栏按钮，弹窗本身也不会再被浮层窗口挡住。
- **拆分手动改稿后的字幕不再错位** ： 字幕文本被人工改写（如把 ASR 词「傲」改成「Alt(noir)」、改错别字、删语气词）导致文本与词时间戳脱钩时，拆分可能在错误的词边界落刀——「字拆对、时拆错」：文本在目标字边界分开，时间却切到别处（后段可能只剩最后一个字且时长暴涨），切点被顶到段尾时还会导致拆分没有反应，刀点左侧替换词的语音也可能被划进右段。现在拆分改用顺序保持的对齐算法，未对齐的词结合相邻词锚点、替换区与刀点的相对位置及其自身时间落边，两侧边界随未对齐词延展，让时间切分贴合下刀处的词边界；全部词都对不上时按文字位置映射词序回退；若文本与词时间戳脱钩且切点明显偏离下刀位置，会给出提示而不是静默出错。
