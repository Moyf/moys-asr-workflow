...
- **错误卡片一键报障** ： 转写等任务失败时，错误卡片的「提交 Issue」会直接打开预填好的 GitHub 新建 Issue 页面，标题带错误码与版本，正文含错误提示和日志，并附复现步骤模板，用户可在提交前继续编辑。日志很长时草稿会自动收敛到 GitHub 能接受的长度，避免预填内容在登录跳转中被丢弃；完整日志仍可通过「复制错误报告」获取，也会写入本机日志文件。系统没有可用浏览器时会如实提示失败原因，并把完整报告放进剪贴板作为出路，不再假装已经打开。
