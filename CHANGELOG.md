...
- MAWE Phase 2 首个纯逻辑切片：将编辑器设置默认值、旧版 `mediaSeekStepSeconds` 兼容迁移、枚举归一化和数值钳制集中到 `editor-utils.js`；`editor.js` 保留 `localStorage` 读写和设置状态装配，设置键名与默认行为不变。
- MAWE Phase 2 追加三个纯逻辑边界：空隙移除设置归一化、历史快照与记录构造、普通/扩展/去空隙 SRT payload；行为、历史类型和导出格式保持不变。
- MAWE Phase 3 建立首批运行环境服务：设置持久化、文件保存与 Blob 下载、Server JSON POST 统一由 `editor-services.js` 注入；file:// 降级和保存框取消仍保持可用，工程/媒体打开与浏览器运行时桥接留待后续切片。
