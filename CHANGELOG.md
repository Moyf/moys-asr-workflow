...
#### 动“帧”格的来了
- **毫秒 / 帧时间基准** ： 编辑器支持按工程设置 FPS 切换毫秒或帧时间轴；帧模式下字幕拖动、边界调整、方向键和 A/D 微调均按帧执行，并以 `HH:MM:SS:FF` 显示时间码，同时保留毫秒兼容字段。
- **波形细网格** ： 波形最大缩放扩展为 2 秒，多行波形也新增 2 秒每行长度；在 2 秒每行长度下按当前时间基准绘制细分竖线网格。

也就是说你现在可以放大到2秒的时间窗口，对30FPS的视频一次查看 60帧 的内容，并精准地按帧为单位来微调字幕。
...
- **多重字幕帧时间保存** ： 帧模式切换或投影后会同步重建绑定 offset，避免保存时报绑定时间偏移不一致。
- **帧时间码保存边界** ： 修复 23.97 等 FPS 下多个字词取整到同一帧后，兼容毫秒时间被扩张到字幕段之外而保存失败的问题。
- **时间码分隔符刷新** ： 修改帧时间码分隔符后立即刷新字幕列表，不必先编辑字幕才看到新的显示格式。
