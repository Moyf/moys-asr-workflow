...
- **桌面编辑器顶栏** ： 隐藏 Electron 默认的 File / Edit / View / Window 菜单，交由 MAWE 自己的主题感知工具栏提供操作入口。

- **字幕预览说话人标签与颜色样式** ： 可为黄、绿、红、紫、蓝五种字幕颜色配置并重命名说话人名称（默认 `SP1`～`SP5`）；开启后仅在播放器预览的字幕前以独立元素显示标签，名称使用对应字幕颜色并继承主字幕字体，不改动工程字幕文本；名称元素包含可自定义的分隔符（默认 `：`，支持空格或英文引号），导出 SRT 时可单独选择是否附加名称和分隔符；预览字幕颜色新增下划线、文字颜色和组合样式选择，空隙 click 不再提前显示拖动样式。
- **工程版本备份** ： Editor「保存」设置新增可选的工程备份，本地 Server 绑定工程可按分钟定时备份，手动保存也创建带时间戳的 `.mosp-bak`；备份放入工程对应的 `_maw/backups`，可设置最多保留的版本数，超出后将最早版本移入回收站。备份不加入「最近工程」，可点击「打开备份文件夹」手动恢复。
- **「视频预览」设置分区** ： 全局设置的「预览字幕」标签页升级为「视频预览」，自上而下分为三组：预览开关组（预览字幕 / 预览副字幕 / 预览表情包 / 自动预览鼠标位置画面，取代媒体区右上角的齿轮面板）、播放控制组（JKL 播放模式与每次跳转时长，自「通用操作」移入）和字幕预览组（原「预览字幕样式」，其中「字幕大小」更名「文字大小」，字体与文字大小各自成行，各设置项样式与窗口其他分区统一）；「预览字幕颜色」不再提供「阴影」样式，旧工程中保存的阴影自动回退为下划线。
- **全局设置窗口缩放** ： 全局设置窗口与帮助窗口一致，可拖动右下角调整窗口大小，并记忆上次尺寸。
- **媒体区与布局编辑视觉** ： 移除媒体预览区顶部的「媒体」标签栏（其功能已并入全局设置，「视频预览」分区可直接管理），画面区域更高；布局编辑模式的拖动分隔线默认隐形、悬停或拖动时才显示，线条从 3px 收窄为 2px；「字幕编辑」区不再常驻顶部留白，进入布局编辑时与媒体区一样自动让出拖动把手高度。
- **主副字幕交换的颜色映射** ： 交换时按绑定关系将主字幕颜色映射到新的主轨，并保留移到副字幕的原主轨 `color` / `color_ref` 信息；颜色引用的 `headIdx` 会按各自字幕数组重新计算。
- **Launcher 高级选项收窄** ： 暂时移除未开放的「时长上限」输入框，底层时长限制参数仍保留。
- **Launcher 教程入口** ： 在「项目官网」旁增加「教程视频」链接，直达 B 站教程视频。
- **字幕导出菜单整理** ： 将完整 SRT、按颜色拆分 SRT 与带样式 ASS 按格式分组；去空隙导出同步提供带样式 ASS；ASS 字号会将手动字号和「自动（响应式）」默认字号按输出视频高度换算，并纳入全屏预览的字号放大校准，避免高分辨率视频中的字幕偏小。
- **任务完成系统通知** ： Launcher「⚙️ 设置 → 通用 → 完成通知」默认关闭；启用后，开关会立即发送一条启用提示，单个文件转写完成或失败、整批队列结束或异常终止时发送系统通知（Windows 气泡提示 / macOS 通知中心 / Linux `notify-send`），批量通知会汇总成功／失败数量。手动停止批量任务不算完成，不会提醒。
- **阿里云百炼 API Key 入口** ： Launcher、CLI 提示和工作流文档改用[阿里云百炼](https://platform.qianwenai.com/home/)获取或查看 API Key。
- **波形振幅按媒体响度自动定标** ： 编辑器首次打开工程时，会用媒体旁 `.quapeaks` / `.ReaPeaks` 的响度层算出整文件电平，自动为波形选定垂直缩放（服务器经 `/api/waveform` 下发 `loudness` 统计）。安静素材不再被画成一条直线，响素材调高振幅后也不再整片削平。标尺刻意取 **响度** 而不是真实峰值：按峰值定标会让一次瞬态就把整条波形压扁，取响度换来的是「大部分时间不越界」。用户手动调过振幅后不再被覆盖（工程 `waveformSettings` 新增 `waveformScaleAuto`，缺失按自动处理，老工程升级即可生效）；波形设置的「振幅」行新增 **响度适配** 按钮可重新采纳自动值，标签以 `×2.4 自动` 标明当前值是算出来的。自动值只作用于当前工程，不写浏览器全局偏好。
- **Apple Silicon 本地识别加速** ： Qwen3-ASR 的自动设备模式在支持时使用 MPS，并让主模型与 Forced Aligner 共用半精度设备配置；MPS 初始化失败时自动回退 CPU。
...
- **自动后处理翻译配置提示** ： 设置页的「大语言模型（AI）」更名为「AI 模型配置」；从「后处理步骤」进入尚未验证的 LLM 配置时，「测试连接」按钮会持续黄色闪烁提醒，点击后立即清除；勾选合并双语字幕时用「双语行序」下拉列表替换说明提示，并更新「只翻译非中文的字幕」说明。工具箱提示保持不变。
...
- **Launcher 错误诊断与脱敏** ： 常见后处理、批量任务、MOSE 和口播对齐错误现在提供中英文操作提示；批量队列逐项保留错误码，编辑器 Server 启动超时会显示进程与启动日志诊断，错误报告和启动日志会自动脱敏常见 API Key、Token、Secret、Password 与 Bearer 凭据。

- **翻译回填与目标语言处理** ： 翻译任务新增与合并双语互斥的「将译文回填到原字幕」输出方式：适用于仅有少量语音需要翻译的情况，翻译结果直接替换原字幕对应句子，只输出单条字幕，未翻译的句子连同逐词时间码原样保留。翻译成中文会跳过已是中文的字幕；翻译成英文会将所有文字字幕交给模型，避免把西语、法语、德语等拉丁文字误判为英文。产物命名中文界面为 `.翻译为中文.回填` / `.后处理.回填`（英文界面 `.translate-zh-backfill` / `.postprocess.backfill`），该命名同样会被重复翻译防护拦截。原「合并双语字幕」选项更名为「将双语字幕合并为单个字幕」，行为不变，并新增「双语行序」可选译文在上或原文在上（默认跟随翻译目标：中文目标译文在上，英文目标原文在上）。
...
- **贴合字幕边界拖动新模式（默认）** ： 两条字幕贴合时，中缝处新增可拖动的边界区，拖动中缝两侧边界一起联动；单独拖动左右手柄仍只调整当前字幕（类似达芬奇的剪辑逻辑）。旧的「自动吸附调整相邻字幕」开关移入「传统模式」，可在「音频波形区 ⚙️ → 操作 → 贴合字幕边界」中切换；键盘微调的默认联动规则不变。
...
- **Windows 应用图标** ： MOSE 打包改用包含多种尺寸的 ICO 资源，修复窗口和任务栏图标缩放后只显示黑色的问题。
- **MOSE 新手引导状态** ： 快速上手的完成 / 跳过状态改为保存在用户级设置中，修复独立编辑器每次使用新 localhost 端口启动都会重复弹出的问题。
...
- **官方 Windows Installer 与更新基础** ： 新增无需管理员权限的 Windows x64 Installer、稳定/Beta 频道检查、ETag 缓存、Release 清单与 SHA-256 校验；Installer 统一包含 MAW + MOSE，`.mosp` 双击先经过 Launcher 更新检查再打开 MOSE，并随公开 GitHub Release 发布。未配置证书时会明确标注为 unsigned。更新过程会保留用户配置、日志和模型缓存。
