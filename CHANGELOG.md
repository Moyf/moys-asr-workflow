...
## [1.5.0-beta.7] - 2026-08-25

### 🚀 全新特性

- **动态图形字幕、文稿匹配与编辑器工作流** ： 汇总 OGraf / Lottie 预览、拖放与时间轴交互、矢量字形导出、文稿匹配预览与配置持久化，以及字幕拆分后的滚动锚点保持。

### 🐛 问题修复

- **跨平台发布与归档验证** ： 兼容 macOS ZIP 对中文 FAQ 文件名的编码差异，改为校验 FAQ 文件内容和应用入口；同时保留 CJK 字体环境、批量转写测试和 AppImage 失败诊断修复。

## [1.5.0-beta.6] - 2026-08-25
...
- **跨平台发布与归档验证** ： 将 macOS 发布包内容校验改为直接读取 ZIP 目录，避免 `unzip`、`grep` 与 `pipefail` 的组合在归档有效时误报失败；同时保留 CJK 字体环境、批量转写测试和 AppImage 失败诊断修复。

## [1.5.0-beta.5] - 2026-08-25
...
- **跨平台发布与归档验证** ： 修复 macOS 发布包在 `set -o pipefail` 下因 `grep -q` 提前退出而被误判失败的问题，并保留 macOS / Linux 的字体环境、批量测试和 AppImage 诊断修复。

## [1.5.0-beta.4] - 2026-08-25
...
- **发布与跨平台兼容性** ： 补齐 macOS / Linux 发布验证所需的 CJK 字体环境，稳定批量转写 API 的跨平台测试，并让 AppImage 诊断在前置构建失败且没有产物时正常跳过。

## [1.5.0-beta.3] - 2026-08-25

### 🚀 全新特性

- **动态图形字幕与编辑器工作流** ： 增加 OGraf / Lottie 的预览、拖放与时间轴交互，并支持将字幕文字转换为内置矢量字形，降低目标环境字体差异；拆分字幕后也会保持列表滚动锚点，连续编辑时视图更稳定。
- **文稿匹配工作流** ： 完善匹配预览、匹配率统计和字幕句数变化提示，并持久化断句符号、匹配模式与自定义提示词配置。

### 🐛 问题修复

- **发布包与跨平台兼容性** ： 补充发布包 FAQ 和 Lite 版 FFmpeg 错误提示，修复空后处理配置阻塞转写，并修正 macOS / Linux 发布验证对系统字体和 AppImage 诊断步骤的依赖。
