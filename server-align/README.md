...
播放时是否跳过 gap 影响当前预览，并作为 `gap_remove.skip_playback` 写入导出工程；Gap 的移除/恢复、边界和位置调整会写入 `gap_remove.gaps`，同时把来源层写入 `gap_remove.provenance`。最终 `gaps[*].source/origins` 是按来源层派生的便捷信息：`source` 保留最初 lineage，`origins` 列出当前区间的全部贡献来源。

Gap 在界面上始终只有一层，但有三种最终状态：`removed: true` 是启用的空隙，会参与跳过播放、导出压缩和字幕禁用；`removed: false` 是人工恢复的空隙，仍保留在时间轴上供查看和再次编辑，但不生效；“清理空隙记录”则从来源层删除选中范围，不留下恢复记录，因此后续重新扫描可能再次生成同一段静音。
