(function initMaweI18n(global) {
  ...
  const EN_TEXT = {
    ...
    '字幕预览中的说话人名称开启后会自动勾选；只影响导出的 SRT，不会改动工程里的字幕文本。': 'This is checked automatically when speaker names are enabled in the subtitle preview; it only affects exported SRT and does not change subtitle text in the project.',
  ...
  const EN_ATTR = {
    ...
    '开启字幕预览中的说话人名称后默认勾选；导出 SRT 时在字幕前附加对应名称，可手动关闭': 'Checked by default when speaker names are enabled in the subtitle preview; add the matching name before each subtitle in exported SRT, and turn it off manually when needed',
    ...
    '配置颜色对应的说话人名称；留空可隐藏该颜色的名称。仅影响预览，不改变字幕文本。': 'Configure the speaker name for each color; leave a name empty to hide it. Preview only, subtitle text is unchanged.',
