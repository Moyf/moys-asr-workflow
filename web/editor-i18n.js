(function initMaweI18n(global) {
  ...
  const EN_TEXT = {
    ...
    '叠加字幕': 'Overlay subtitles',
    '显示独立叠加字幕轨；它不会与多重字幕建立绑定关系': 'Show the independent overlay subtitle track; it does not bind to multi-subtitles',
    ...
    '多重字幕的副字幕在 ASS 导出与「ASS 字幕模式」预览中共用此样式。': 'Multi-subtitle extension cues share this style in ASS export and "ASS subtitle mode" preview.',
    ...
    'ASS 字幕模式下副字幕使用的样式；多重字幕的副字幕共用一个样式，与样式库「副字幕样式」槽位同步': 'The style used by extension subtitles in ASS subtitle mode; multi-subtitle extension cues share one style, in sync with the "Extension-subtitle style" slot in the style library',
