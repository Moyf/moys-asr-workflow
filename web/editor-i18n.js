(function initMaweI18n(global) {
  ...
  const EN_TEXT = {
    ...
    '需要启用「将颜色映射为说话人」；勾选「': 'Enable “Map colors to speakers”; check “',
    '」。': '”.',
    ...
    '自定义…': 'Custom…', '宽度': 'Width', '高度': 'Height',
    '自定义合成尺寸需要 16–7680 之间的整数宽高': 'Custom canvas size needs integer width and height between 16 and 7680',
    ...
    '导出字幕轨': 'Export subtitle tracks', '叠加轨字幕': 'Overlay-track subtitles',
    '全部字幕轨（各占一轨）': 'All subtitle tracks (one per track)',
    ...
    '有颜色的字幕会使用对应颜色覆盖主色/描边，取决于': 'Colored subtitles override the primary color/stroke with the matching color, depending on',
    '中的设置。': 'the setting in',
    '将「字幕样式」中的预览字体应用到 ASS 字体': 'Apply the preview font from subtitle styles to the ASS font',
    '去除间隙': 'Remove gaps',
    ...
    '原生文本仅作为可选交接数据，不承诺样式或位置还原；SRT 可通过独立按钮导出。未写入原生文本时「导出字幕轨」不生效。': 'Native text is optional handoff data with no style or placement fidelity claim; export SRT with its separate button. Subtitle track choice has no effect unless native text is enabled.',
    '导出 XML': 'Export XML',
    ...
    '双语字幕': 'Bilingual subtitles', '双语字幕设置': 'Bilingual-subtitle settings', '主轨': 'Main track', '副轨': 'Secondary track', '双列': 'Two columns', '绑定字幕后自动把副字幕的起止时间同步到主字幕，相当于随后按一次 H': 'After binding, sync the secondary subtitle start and end to the main subtitle, equivalent to pressing H', '交换主字幕和副字幕的文本、时间与绑定关系': 'Swap the main and secondary subtitle text, timing, and bindings',
    '拆分与合并': 'Split and merge', '拆分与合并配置': 'Split and merge settings', '波形形状来源': 'Waveform shape source', '原生波形': 'Native waveform', 'REAPER 波形': 'REAPER waveform', '被禁用字幕的波形显示方式': 'How disabled subtitles appear on the waveform',
    '显示方式': 'Display mode', '语言类型': 'Language type', '字幕语言类型': 'Subtitle language type', '主字幕': 'Main subtitle', '副字幕': 'Secondary subtitle', '主字幕语言类型': 'Main subtitle language type', '副字幕语言类型': 'Secondary subtitle language type', '副字幕时波形高度': 'Waveform height with secondary subtitles', '跨轨道吸附': 'Cross-track snapping', '同时选中主副字幕': 'Select main and secondary subtitles together', '绑定时自动同步时长': 'Automatically sync duration when binding', '显示轨道徽标': 'Show track badges', '在双语字幕波形中显示主字幕和副字幕的轨道编号徽标': 'Show main and secondary track number badges in the bilingual waveform', '交换主副字幕': 'Swap main and secondary subtitles', '普通点击以最后点击的轨道为准；点击已绑定字幕时，仅补选它实际绑定的另一条字幕': 'Normal clicks follow the last clicked track; clicking a bound subtitle only adds the other subtitle actually bound to it',
    ...
    '启用双语字幕时使用的波形行高度': 'Waveform row height used when bilingual subtitles are enabled', '请拖入第二个 srt 字幕以开启双语字幕功能': 'Drop a second SRT subtitle to enable bilingual subtitles', '请拖入第二条字幕以开启双语字幕编辑': 'Drop a second subtitle to enable bilingual subtitle editing', '是否导入第二条字幕？（后续也可以将字幕或工程拖入编辑器加载）': 'Import the second subtitle? (You can also drop a subtitle or project file into the editor later)', '当前工程如果有大于1条字幕，可以开启双语字幕模式，用于双语字幕编辑等。': 'When the current project has more than one subtitle, you can enable bilingual-subtitle mode for bilingual subtitle editing and similar workflows.',
    '拖动副字幕时，允许吸附到主字幕轨道的起点和终点': 'Snap secondary subtitles to the main track\'s start and end boundaries while dragging',
    ...
    '请先开启双语字幕': 'Enable bilingual subtitles first',
    ...
    '⬆️ 当前预览样式由 ASS 字幕模式控制': '⬆️ Current preview styling is controlled by ASS subtitle mode',
    '使用 ASS 样式': 'Use ASS style', '主字幕 ASS 样式': 'Main-subtitle ASS style', '副字幕 ASS 样式': 'Extension-subtitle ASS style',
    '副字幕样式': 'Extension-subtitle style', 'ASS 副字幕样式': 'ASS extension subtitle style', 'ASS 副字幕默认样式': 'Default style for the ASS extension subtitle',
    '双语字幕的副字幕在 ASS 导出与「ASS 字幕模式」预览中共用此样式。': 'Bilingual-subtitle extension cues share this style in ASS export and "ASS subtitle mode" preview.',
    'ASS 字幕模式下主字幕使用的样式；即当前 ASS 输出方案关联的样式，与样式库中的选择同步': 'The style used by the main subtitle in ASS subtitle mode; it is the style linked to the current ASS export profile and stays in sync with the style library',
    'ASS 字幕模式下副字幕使用的样式；双语字幕的副字幕共用一个样式，与样式库「副字幕样式」槽位同步': 'The style used by extension subtitles in ASS subtitle mode; bilingual-subtitle extension cues share one style, in sync with the "Extension-subtitle style" slot in the style library',
    ...
    '开启后，播放器预览尽量按当前 ASS 默认方案渲染，只保留字幕文本中的显式换行；关闭时保持原有预览效果': 'When enabled, the player approximates the current ASS default profile and keeps only explicit subtitle line breaks; when disabled, the original preview is kept',
    '关闭时保持原有 CSS 预览；开启后使用 ASS 的字体、颜色、粗体、斜体、下划线、描边、阴影、对齐和动画预览，并只保留字幕文本中的显式换行，不自动折行。': 'When disabled, the original CSS preview is kept; when enabled, the player previews ASS font, color, bold, italic, underline, outline, shadow, alignment, and animations while keeping only explicit subtitle line breaks.',
    '管理 ASS 样式': 'Manage ASS styles', '打开 ASS 样式库': 'Open the ASS style library',
    '编辑样式': 'Edit style', '打开样式库并定位到当前选中的样式': 'Open the style library at the selected style',
    ...
    '＋ 新建方案': '+ New profile', '默认槽位': 'Default slots', 'SRT 压制': 'SRT burn-in', 'SRT 默认烧录样式': 'Default SRT burn-in style', 'SRT 烧录': 'SRT burn-in',
    ...
    'ASS 导出使用此方案及其动画；双语字幕的副字幕样式在该方案的表单中配置。': 'ASS export uses this profile and its animations; for bilingual subtitles, the extension-subtitle style is configured in the profile form.',
    '次色仅在 \\k 卡拉 OK 标签中使用，当前未支持': 'Secondary color is only used by \\k karaoke tags; not supported yet',
    '主字幕样式': 'Main subtitle style', '副字幕默认样式': 'Default extension-subtitle style',
    '从左侧选择一个样式或 ASS 方案。': 'Choose a style or ASS profile on the left.', 'STYLE / PROFILE': 'STYLE / PROFILE',
    '名称': 'Name', '字号': 'Font size', '字距': 'Letter spacing', '颜色': 'Colors', '主': 'Main', '副': 'Secondary', '主色': 'Primary', '次色': 'Secondary',
    ...
    '阴影距离': 'Shadow distance', '边框样式': 'Border style', '不透明底框': 'Opaque box', '左下': 'Bottom left', '中下': 'Bottom center',
    ...
    '在这里配置的覆盖标签会应用到每一句字幕上，例如批量淡入淡出。': 'Override tags configured here are applied to every cue, for example batch fade in and fade out.',
    ...
    '自定义 ASS 标签（不含大括号）': 'Custom ASS tags (without braces)', '进阶动画': 'Advanced animations',
    ...
    '自定义颜色色值': 'Custom color values', '恢复默认': 'Restore defaults',
    ...
    '播放过程中点击鼠标自动暂停': 'Pause automatically when clicking the mouse during playback',
    '开启后，播放过程中通过鼠标点击字幕或波形跳转时会暂停；暂停时不受影响': 'When enabled, mouse clicks that seek from subtitles or the waveform pause playback; clicks while paused are unchanged',
    '播放时点击字幕、波形或空隙跳转后暂停；暂停时的点击行为不变。': 'While playing, clicking a subtitle, the waveform, or a gap seeks and then pauses; click behavior while paused is unchanged.',
    ...
    '双语字幕列表显示方式': 'Bilingual-subtitle list display mode', '英文、西文等按空格拆分请选择「单词型」；中文、日文等按字符拆分请选择「字符型」。': 'Choose Word-based for English and other space-separated languages; choose Character-based for Chinese, Japanese, and other character-separated languages.',
    ...
    '替换当前字幕': 'Replace current subtitles', '作为副字幕': 'Use as secondary subtitles', '导入': 'Import',
    ...
    '单词型语言（英语等）只在空格处切分，确保不会拆碎单词；可在双语字幕 ⚙️ 设置中切换语言类型。': 'Word-based languages such as English split only at spaces so words stay intact; switch the language type in the bilingual-subtitle ⚙️ settings.',
    '移动鼠标并点击选择的拆分断点；字符型语言（中文等）按字符拆分，单词型语言（英语等）只在空格处切分，确保不会拆碎单词。你可以在双语字幕设置中切换语言类型。': 'Move the mouse and click to choose a split boundary. Character-based languages such as Chinese split by character; word-based languages such as English split only at spaces so words stay intact. Change the language type in the bilingual-subtitle settings.',
    ...
    '拆分并保留原文': 'Split and keep the original text', '拆分后两边保留相同的字幕文本': 'Keep the same subtitle text on both sides after splitting',
    ...
    '「通用操作」中的「空隙区段操作方式」，其中「边界与中键」可同时使用两套操作。': '“Gap region operation” under “General” in Global settings; “Boundary and middle” enables both operation sets.',
    ...
    '注：微调幅度可在': 'Note: Adjust the fine-tuning amount in',
    '「通用操作」中调节，默认 50ms': 'under “General” in Global settings; the default is 50 ms',
    '其他': 'Other',
    ...
    '转为主字幕': 'Convert to main subtitle', '转为叠加字幕': 'Convert to overlay subtitle',
    ...
    '禁用波形显示': 'Disable waveform display', '淡化': 'Dim', '完全隐藏': 'Hide completely',
  ...
  const EN_ATTR = {
    ...
    '打开导出设置并定位到说话人名称选项': 'Open Export settings and locate the speaker-name option',
    '开启字幕预览中的说话人名称后默认勾选；导出 SRT 时在字幕前附加对应名称及分隔符，可手动关闭': 'Checked by default when speaker names are enabled in the subtitle preview; add the matching name and separator before each subtitle in exported SRT, and turn it off manually when needed',
    '在播放器预览的字幕前显示颜色对应的说话人名称；只影响预览，不改变字幕文本': 'Show the speaker name for each color before the subtitle in the player preview; preview only, subtitle text is unchanged',
    '配置颜色对应的说话人名称；留空可隐藏该颜色的名称。分隔符默认「：」，支持空格和英文引号。仅影响预览，不改变字幕文本。': 'Configure the speaker name for each color; leave a name empty to hide it. The separator defaults to “：” and supports spaces or English quotation marks. Preview only, subtitle text is unchanged.',
    '设置说话人名称与字幕内容之间的分隔符；默认「：」，也可以使用空格或英文引号': 'Set the separator between the speaker name and subtitle text; the default is “：”, and spaces or English quotation marks are also supported',
    '设置黄色对应的说话人名称': 'Set the speaker name for yellow',
    '设置绿色对应的说话人名称': 'Set the speaker name for green',
    '设置红色对应的说话人名称': 'Set the speaker name for red',
    '设置紫色对应的说话人名称': 'Set the speaker name for purple',
    '设置蓝色对应的说话人名称': 'Set the speaker name for blue',
    '黄色对应的说话人名称': 'Speaker name for yellow',
    '绿色对应的说话人名称': 'Speaker name for green',
    '红色对应的说话人名称': 'Speaker name for red',
    '紫色对应的说话人名称': 'Speaker name for purple',
    '蓝色对应的说话人名称': 'Speaker name for blue',
    ...
    '双语字幕的设置': 'bilingual-subtitle settings', '中单独配置两种字幕的语言类型。': ' separately.',
    '对于双语字幕，可以在双语字幕的设置（需要先开启双语字幕）中单独配置两种字幕的语言类型。': 'For dual-track subtitles, configure each track\'s language type in bilingual-subtitle settings (enable bilingual subtitles first).',
    '打开双语字幕设置': 'Open bilingual-subtitle settings',
