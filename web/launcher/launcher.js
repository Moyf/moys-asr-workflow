(function () {
  ...
  const state = { lang: "zh", serverRunning: false, serverStarting: false, running: false, result: null, config: null, srtAuto: true, serverMediaOk: false };
  ...
  function serverPayload() { return { jsonPath: $("jsonPath").value.trim(), mediaPath: $("serverMediaPath").value.trim(), port: $("port").value || "8765", guiLang: state.lang }; }
  function renderMaweButton() {
    const button = $("openMawe");
    button.textContent = state.serverStarting
      ? SERVER_STARTING_TEXT[state.lang]
      : (state.serverRunning ? t("server_stop") : t("open_mawe"));
  ...
  async function openMawe() {
    ...
    try {
      ...
      const result = state.serverRunning ? await bridge("stop_server") : await bridge("start_server", serverPayload());
      if (result.ok) {
        state.serverRunning = !state.serverRunning;
        renderMaweButton();
        setStatus(result.url || t("ready"));
      } else {
