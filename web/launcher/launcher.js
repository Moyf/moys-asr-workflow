(function () {
  ...
  const STRINGS = {
    zh: { media_output: "① 媒体与输出", recognition: "② 识别设置", server: "④ 编辑器服务器", logs: "③ 日志", provider: "语音 API 供应商", test_run: "测试运行", test_run_title: "仅截取前2分钟内容，用于测试功能和 API", test_run_override: "测试运行已限定前 2 分钟", hero_desc: "本地媒体 -> AI 转写 -> 可编辑字幕工程", project_home: "项目官网", media: "媒体文件", srt_output: "SRT 输出", choose: "选择", model: "模型", region: "地域", workspace: "工作空间 ID", language: "语言", length_limit: "时长上限", language_reset: "重置（自动识别）", language_multi_hint: "可多选；不选即自动识别（仅偏向，不限制）；默认仅显示常用语言，其余可在「配置 → 显示小语种」开启。", settings_language: "语言", show_rare_langs: "显示小语种", show_rare_langs_hint: "开启后，「语言」列表显示供应商支持的全部语种；关闭时只显示常用语言。", key: "API Key", save_key: "存入本地环境", key_hint_prefix: "在", key_hint_suffix: "获取 API Key ↗", json_project: "工程 JSON", json_placeholder: "生成工程后会自动填入，也可以手动选择之前的工程", server_media: "服务器媒体（可选）", server_media_missing: "工程未记录媒体，或文件已移动，请手动选择。", port: "端口", advanced: "高级选项", open_mawe: "启动字幕编辑器", server_stop: "停止服务器", start: "生成字幕和工程", open_folder: "打开输出文件夹", open_html: "打开 html 编辑器", open_blank_html: "打开 html 空模板", demo_mode: "演示模式", settings_title: "配置", settings_ffmpeg: "FFmpeg", settings_stickers: "默认表情包路径", stickers_explain: "表情包根目录供 HTML 编辑器使用；支持嵌套子目录（如 大狗/、Nox/ 等）。", current_value: "当前", unset: "未设置", sticker_dir: "表情包根目录", choose_folder: "选择文件夹", change: "更改", ffmpeg_found: "成功定位到 ffmpeg", ffmpeg_path: "FFmpeg 路径", ffmpeg_placeholder: "ffmpeg.exe / ffprobe.exe 所在 bin 目录，或 ffmpeg.exe", ffmpeg_help: "如何安装 FFmpeg ↗", ffmpeg_missing: "未找到 ffmpeg / ffprobe", ffmpeg_need: "需要依赖 ffmpeg 先将视频转成音频后才能发送给服务器转录", sticker_missing: "请选择一个存在的文件夹。", ready: "就绪", running: "转写中…", saved: "设置已保存", failed: "失败", done: "完成", key_empty: "未配置密钥", key_loaded: "已加载密钥 {key}", workspace_hint: "新加坡地域需要填写 Workspace ID。", other_language: "English", drop_hint: "拖入音频/视频文件，或点击选择。", drop_reject: "只支持音频、视频或 JSON 工程文件。", media_required: "请选择存在的媒体文件。", output_required: "请填写 SRT 输出路径。", key_required: "请填写 API Key，或先保存到 .env。", workspace_required: "新加坡地域需要 Workspace ID。", json_required: "请选择工程 JSON 后再打开 MAWE。", server_media_required: "工程没有可用媒体，请手动选择媒体文件。", speaker_colors: "给不同说话人分配字幕颜色", speaker_colors_hint: "最多 5 种颜色；说话人超过 5 个时颜色循环复用。", speaker_colors_title: "转写时按说话人自动着色（生成后仍可在编辑器修改）" },
    en: { media_output: "① Media & Output", recognition: "② Recognition Settings", server: "④ Editor Server", logs: "③ Logs", provider: "Speech API Provider", test_run: "Test run", test_run_title: "Trim to the first 2 minutes to test the workflow and API", test_run_override: "Test run is limited to the first 2 minutes", hero_desc: "Local media -> AI transcription -> Editable subtitle projects", project_home: "Project", media: "Media file", srt_output: "SRT output", choose: "Choose", model: "Model", region: "Region", workspace: "Workspace ID", language: "Language", length_limit: "Length limit", language_reset: "Reset (auto-detect)", language_multi_hint: "Multi-select; empty = auto (bias only). Rare languages are hidden — enable them in Settings → Show rare languages.", settings_language: "Language", show_rare_langs: "Show rare languages", show_rare_langs_hint: "When enabled, the language list shows every supported language; otherwise only common ones.", key: "API Key", save_key: "Save locally", key_hint_prefix: "Get an API Key from", key_hint_suffix: "↗", json_project: "Project JSON", json_placeholder: "Auto-filled after generation, or choose an earlier project", server_media: "Server media (optional)", server_media_missing: "The project has no media, or the file moved. Choose it manually.", port: "Port", advanced: "Advanced options", open_mawe: "Launch Subtitle Editor", server_stop: "Stop server", start: "Generate subtitles + project", open_folder: "Open output folder", open_html: "Open HTML editor", open_blank_html: "Open blank HTML template", demo_mode: "Demo mode", settings_title: "Settings", settings_ffmpeg: "FFmpeg", settings_stickers: "Default sticker path", stickers_explain: "Sticker root directory for the HTML editor; nested folders are supported.", current_value: "Current", unset: "Not set", sticker_dir: "Sticker root", choose_folder: "Choose folder", change: "Change", ffmpeg_found: "Located ffmpeg successfully", ffmpeg_path: "FFmpeg path", ffmpeg_placeholder: "bin directory containing ffmpeg/ffprobe, or ffmpeg executable", ffmpeg_help: "How to install FFmpeg ↗", ffmpeg_missing: "ffmpeg / ffprobe not found", ffmpeg_need: "ffmpeg is required to convert video to audio before sending it to the transcription server", sticker_missing: "Choose an existing folder.", ready: "Ready", running: "Running…", saved: "Settings saved", failed: "Failed", done: "Done", key_empty: "No key configured", key_loaded: "Loaded key {key}", workspace_hint: "Singapore requires a Workspace ID.", other_language: "中文", drop_hint: "Drop an audio/video file here, or choose one.", drop_reject: "Only audio, video, or JSON project files are supported.", media_required: "Choose an existing media file.", output_required: "Enter an SRT output path.", key_required: "Enter an API key, or save one to .env first.", workspace_required: "Workspace ID is required for Singapore.", json_required: "Choose a project JSON before opening MAWE.", server_media_required: "The project has no usable media. Choose media manually.", speaker_colors: "Assign subtitle colors to speakers", speaker_colors_hint: "Up to 5 colors; colors cycle when there are more than 5 speakers.", speaker_colors_title: "Color subtitles by speaker during transcription (editable afterwards)" }
  };

  const MEDIA_EXTS = new Set([".mp4", ".mkv", ".avi", ".mov", ".wmv", ".flv", ".webm", ".ts", ".m4v", ".mp3", ".wav", ".m4a", ".flac", ".aac", ".ogg"]);
  const ERROR_TEXT = {
    zh: {
      json_not_found: "工程 JSON 不存在，请检查路径。",
    ...
    en: {
      json_not_found: "Project JSON does not exist. Check the path.",
  ...
  const state = { lang: "zh", serverRunning: false, running: false, result: null, config: null, srtAuto: true, serverMediaOk: false };
  ...
  function mockApi() {
    ...
    return {
      get_config: async () => ({ apiKey: saved.apiKey, maskedApiKey: saved.apiKey ? "sk-…demo" : "", providerId: "qwen", modelId: "qwen3-asr-flash-filetrans", lastModel: localStorage.getItem(LAST_MODEL_KEY), lastLanguage: localStorage.getItem(LAST_LANGUAGE_KEY), region: saved.region, language: saved.language, workspaceId: saved.workspaceId, guiLang: saved.guiLang, showRareLangs: saved.showRareLangs || false, appVersion: "1.1.0", stickerDir: saved.stickerDir || "", providers: [{ id: "qwen", label: "Qwen ASR（阿里云百炼）", keyUrl: "https://help.aliyun.com/zh/model-studio/get-api-key", apiKey: saved.apiKey, maskedApiKey: saved.apiKey ? "sk-…demo" : "", supportsSpeaker: false, multiLanguage: false, models: [{ id: "qwen3-asr-flash-filetrans", label: "Qwen3 ASR（文件转写）", envKey: "DASHSCOPE_API_KEY", note: "" }], regions: [{ id: "beijing", label: "北京（华北 2，默认）" }, { id: "singapore", label: "新加坡（需要 Workspace ID）" }], languages: [{ id: "", label: "自动识别" }, { id: "zh", label: "中文 / Mandarin" }, { id: "en", label: "英语 / English" }] }, { id: "soniox", label: "Soniox STT", keyUrl: "https://console.soniox.com", apiKey: "", maskedApiKey: "", supportsSpeaker: true, multiLanguage: true, commonLanguages: ["zh", "en", "ja", "ko"], models: [{ id: "stt-async-v5", label: "Soniox Async STT（v5）", envKey: "SONIOX_API_KEY", note: "" }], regions: [], languages: [{ id: "zh", label: "中文 / Mandarin" }, { id: "en", label: "英语 / English" }, { id: "ja", label: "日语 / Japanese" }, { id: "ko", label: "韩语 / Korean" }, { id: "fr", label: "法语 / French" }, { id: "de", label: "德语 / German" }] }] }),
      default_output: async ({ mediaPath, providerId }) => ({ ok: true, path: mediaPath ? mediaPath.replace(/\.[^.\\/]+$/, providerId === "soniox" ? ".soniox.srt" : ".qwen3-asr-api.srt") : "" }),
      ...
      start_server: async () => { setTimeout(() => window.MAWLauncher.onBackendEvent({ type: "log", message: "[mock] would open http://127.0.0.1:8765/ after server responds" }), 120); return { ok: true, url: "http://127.0.0.1:8765/" }; },
  ...
  function applyProvider(persistReset = false) { const current = provider(); const preferred = state.config.lastModel; const fallback = state.config.modelId || current.models[0]?.id; const modelValue = current.models.some((item) => item.id === preferred) ? preferred : (current.models.some((item) => item.id === fallback) ? fallback : current.models[0]?.id); fillSelect("model", current.models, modelValue); if (persistReset && modelValue) savePrefsDebounced({ modelId: modelValue }); fillSelect("region", current.regions, state.config.region || "beijing"); applyProviderLanguages(current, persistReset); $("openKeyUrl").textContent = current.label; $("regionField").classList.toggle("hidden", current.regions.length === 0); $("speakerColorsField").classList.toggle("hidden", !current.supportsSpeaker); $("apiKey").value = current.apiKey || ""; renderKeyStatus(); syncWorkspace(); syncDefaultOutput(); }
  function applyProviderLanguages(current, persistReset = false) { const el = $("language"); const previous = el.multiple ? Array.from(el.selectedOptions).map((o) => o.value) : (el.value ? [el.value] : []); const remembered = state.config.lastLanguage; const wanted = previous.length && persistReset ? previous : (remembered !== null && remembered !== undefined ? (remembered ? remembered.split(",") : []) : [state.config.language].filter(Boolean)); el.multiple = Boolean(current.multiLanguage); if (current.multiLanguage) el.size = 6; else el.removeAttribute("size"); const showRare = Boolean(state.config.showRareLangs); const commons = current.commonLanguages || []; const visible = current.multiLanguage && !showRare && commons.length ? current.languages.filter((item) => commons.includes(item.id) || wanted.includes(item.id)) : current.languages; fillSelect("language", visible, ""); const codes = new Set(visible.map((item) => item.id)); const restored = wanted.filter((code) => code && codes.has(code)); if (current.multiLanguage) { Array.from(el.options).forEach((o) => { o.selected = restored.includes(o.value); }); } else { el.value = restored[0] || ""; } if (persistReset) savePrefsDebounced({ language: languageValue() }); $("languageHint").classList.toggle("hidden", !current.multiLanguage); $("languageReset").classList.toggle("hidden", !current.multiLanguage); }
  ...
  function syncWorkspace() { const hasRegions = provider().regions.length > 0; $("workspaceField").classList.toggle("hidden", !(hasRegions && $("region").value === "singapore")); }
  ...
  async function syncDefaultOutput() { const result = await bridge("default_output", { mediaPath: $("mediaPath").value.trim(), providerId: $("provider").value }); const path = result.ok ? result.path : ""; $("srtPath").placeholder = path; if (state.srtAuto) { $("srtPath").value = path; if (path) setError("srtPath", ""); } }
  ...
  async function openMawe() { clearErrors(); $("openMawe").classList.remove("attention"); if ($("jsonPath").value.trim()) { const mediaState = await refreshServerMedia(); if ((!mediaState.hasMedia || !mediaState.mediaExists) && !$("serverMediaPath").value.trim()) { expandServer(); return fail("serverMediaPath", errText("server_media_missing", "")); } } const result = state.serverRunning ? await bridge("stop_server") : await bridge("start_server", serverPayload()); if (result.ok) { state.serverRunning = !state.serverRunning; renderMaweButton(); setStatus(result.url || t("ready")); } else { applyErrorResult(result); } }
  ...
  async function init() {
    ...
    appendLog(window.MAWLauncher.backend === "real" ? "MAW launcher ready." : "[mock] Static browser demo mode enabled."); setStatus(t("ready"));
  }

  function handleBackendEvent(event) { if (event.type === "log") appendLog(event.message); if (event.type === "error") { setRunning(false); setStatus(event.message || t("failed")); } if (event.type === "done") { state.result = event.result; setRunning(false); setStatus(t("done")); setJsonPath(event.result?.jsonPath || ""); $("openMawe").classList.add("attention"); $("openFolder").classList.remove("hidden"); $("openHtml").disabled = !event.result?.htmlPath; appendLog(t("done")); } if (event.type === "dropMedia") { setMedia(event.path || ""); setStatus(t("media")); } if (event.type === "dropJson") { setJsonPath(event.path || ""); setStatus(t("json_project")); } if (event.type === "dropReject") setStatus(t("drop_reject")); }
  ...
  ["apiKey", "workspaceId", "serverMediaPath", "port", "ffmpegPath", "stickerDir"].forEach((field) => { const el = $(field); el?.addEventListener("input", () => setError(field, "")); el?.addEventListener("change", () => setError(field, "")); });
  ...
  $("pickStickerDir").addEventListener("click", async () => { const result = await bridge("choose_folder"); if (result.ok) $("stickerDir").value = result.path; });
  $("saveStickerDir").addEventListener("click", async () => { const result = await bridge("save_sticker_dir", { path: $("stickerDir").value.trim() }); setError("stickerDir", result.ok ? "" : errText(result.code, result.detail || result.error)); if (result.ok) { state.config.stickerDir = result.stickerDir; renderStickerCurrent(); setStatus(t("saved")); } else setStatus(errText(result.code, result.detail || result.error)); });
  $("showRareLangs").addEventListener("change", async () => { state.config.showRareLangs = $("showRareLangs").checked; applyProviderLanguages(provider()); await bridge("save_prefs", { showRareLangs: state.config.showRareLangs }); setStatus(t("saved")); });
  ...
  $("openMenu").addEventListener("click", () => $("htmlMenu").classList.toggle("hidden")); $("openHtml").addEventListener("click", () => { $("htmlMenu").classList.add("hidden"); bridge("open_html"); }); $("openBlankHtml").addEventListener("click", () => { $("htmlMenu").classList.add("hidden"); bridge("open_blank_html"); });
