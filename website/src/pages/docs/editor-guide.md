...
位置与大小以归一化比例保存在工程文件的 `preview.subtitle`，因此播放器或窗口改变大小后仍保持相对位置。设置中的“字幕预览”还可以选择主字幕和副字幕的字号、字体、颜色，并在多重字幕开启时用分隔线区分副字幕设置；面板会保持足够宽度以完整显示这些选项。波形设置中的“频谱颜色”默认关闭，关闭时使用纯色波形，打开后才使用频谱缓存着色。字号缺失时沿用响应式默认值；导出 ASS 时会将这个默认值按 1080p 基准换算到输出视频高度。一次拖动或缩放只产生一条撤销记录；这些设置只改变预览呈现，绝不修改字幕或字词时间。
