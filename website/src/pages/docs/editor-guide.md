...
在“预览字幕样式”中开启“显示说话人名称”后，可以为黄、绿、红、紫、蓝五种标记颜色分别配置名称，默认是 `SP1`～`SP5`。播放器预览会在字幕前显示对应名称，例如 `SP1: 字幕内容`；该前缀不写入工程的 `segments[*].text`。开启后，“导出时附加说话人名称”会自动勾选，导出 SRT 时才会真正附加前缀；这个导出选项仍可手动关闭。
