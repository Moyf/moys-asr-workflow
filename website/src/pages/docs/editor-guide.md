...
字幕时间的微调快捷键（包括选中字幕时的方向键、`Shift` 贴合，以及按住波形字幕块时的 `A` / `D` 调整）单独整理在 [字幕按键调整文档](../keyboard-adjustment/) 中。
...
位置与大小以归一化比例保存在工程文件的 `preview.subtitle`，因此播放器或窗口改变大小后仍保持相对位置。设置中的“字幕预览”还可以选择主字幕和副字幕的字号、字体、颜色，并在多重字幕开启时用分隔线区分副字幕设置；面板会保持足够宽度以完整显示这些选项。波形设置中的“频谱颜色”默认关闭，关闭时使用纯色波形，打开后才使用频谱缓存着色。字号缺失时沿用响应式默认值；导出 ASS 时会以 1080p 的默认字号 `72` 为基准，按输出视频高度换算，并按相同比例处理手动字号。一次拖动或缩放只产生一条撤销记录；这些设置只改变预览呈现，绝不修改字幕或字词时间。

在“预览字幕样式”中开启“显示说话人名称”后，可以为黄、绿、红、紫、蓝五种标记颜色分别配置名称，默认是 `SP1`～`SP5`。播放器预览会在字幕前显示对应名称，例如 `SP1: 字幕内容`；该前缀不写入工程的 `segments[*].text`。开启后，“导出时附加说话人名称”会自动勾选，导出 SRT 或 ASS 时才会真正附加前缀；这个导出选项仍可手动关闭。
