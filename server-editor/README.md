...
服务器会在本机保存最近 10 个**明确打开过**的 JSON 工程（路径和文件名），工具栏的「最近工程」可直接切换；第一项标为「上次打开」。切换时服务器重新按工程 JSON 的 `media` 字段、或同目录同名前缀的媒体文件规则加载媒体，不会扫描任何目录。带 JSON 路径启动服务器、另存为、或从最近列表切换都会更新记录；浏览器的普通文件选择器无法提供真实本机路径，因此它仍只是一次性导入。
...
- `--blank` 没有可安全写回的源路径，因此继续使用「导出工程」下载 JSON。
