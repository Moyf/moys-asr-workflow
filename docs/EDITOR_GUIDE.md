...
播放器播放时会高亮当前字幕，字幕列表默认跟随播放头滚动；用滚轮、触摸或方向键手动翻看列表会暂停跟随，点击列表工具栏的「跟随播放」按钮可回到播放头并恢复跟随，空格播放／暂停不改变跟随状态。编辑、拆分、合并及撤销重做时列表保持当前阅读位置。播放器下方的独立控制栏提供播放/暂停、前后 5 秒、进度、音量、倍速和全屏；它不会覆盖画面内的字幕预览。单击字幕列表或波形上的字幕块，会跳到该字幕的开始位置；双击波形可播放或暂停。拖动波形上的字幕块可整体移动字幕，拖动两端可调整开始和结束时间。
