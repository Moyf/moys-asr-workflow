...
- **SRT / ASS / TXT**：SRT 用于播放器、剪辑软件和普通字幕交付；可选择把首条字幕的起点拉到 0（只延长首条，不改变其结束时间或后续字幕时间码）。ASS 使用主字幕预览当前选中的字体、字号和文字颜色写入默认样式，适合交给支持样式字幕的播放器或剪辑软件；位置、描边和背景采用通用底部居中样式，不写入预览框几何或每条字幕的颜色分组。工程存在颜色标记时，可导出完整字幕，或按每种已使用颜色（含无颜色的默认组，中文界面文件名后缀为「无色」，英文界面为 `default`）分别生成带颜色名后缀的 SRT。也可导出逐字幕行排列的纯文本 TXT。
