...
“移除”是非破坏性的：不会剪掉原视频、不会改写原始字幕时间。它会在工程文件的 `gap_remove` 字段中保存可撤销决定，并建立一条压缩时间线：播放时可跳过已移除空隙，导出时可生成去空隙 SRT、OTIO、视频/音频 EDL、FFconcat 或保留区域 JSON。

波形中橙色斜纹代表已移除，灰蓝斜纹代表被保留的空隙。`Alt + 点击` 可切换单段状态；在波形空白处右键选择“添加空隙”会从鼠标位置插入一段默认时长的移除区段；在波形空白处按住 `Alt` 用左键拖动，可直接按范围增加新的空隙。选择“拖动边界”或“边界与中键”时可拖动左右边界；选择“中键拖动”或“边界与中键”时可用中键调整范围。空隙块现在可直接用左键拖动整体偏移，按住 `Alt` 拖动仍兼容整体偏移，`Ctrl/Cmd + 拖动` 可复制 Gap；更细的范围调整在“空隙操作”设置中启用。

“空隙检测与调整”中的“收缩空隙”会按当前前端/后端预留，把已有空隙的左右边界额外向内调整；它不会改变扫描生成时的预留逻辑，适合对现有结果做微调，点击后仍可撤销。重复点击会继续执行一次额外收缩。
...
- **去空隙导出**：仅在有已移除空隙时出现，包括完整或按颜色拆分的 SRT、时间线 OTIO、视频/音频 EDL、FFconcat 和保留区域 JSON。
- **表情包 OTIO**：将已分配的表情包输出为独立图片轨道时间线。
- **视频/音频 EDL**：在“去空隙”菜单中导出原视频或音频的 CMX 3600、30fps、非丢帧 EDL。每个保留的源媒体区间都有源入点/出点，并按顺序拼接到去空隙记录时间线；视频会同时写入 `V` 与 `AA` 事件，纯音频只写入 `AA` 事件。EDL 只引用原媒体路径，不会复制或重新编码媒体。
