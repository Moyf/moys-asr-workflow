...
在“字幕颜色”中开启“将颜色映射为说话人”后，会显示“在预览字幕中显示说话人”和颜色/说话人映射。开启显示后，可以为黄、绿、红、紫、蓝五种标记颜色分别配置名称，默认是 `SP1`～`SP5`。播放器预览会在字幕前显示对应名称，例如 `SP1：字幕内容`；名称和分隔符放在独立元素中，且不写入工程的 `segments[*].text`。设置中的“分隔符”默认为全角冒号 `：`，也可以改为空格或英文引号。开启后，“导出时附加说话人名称”会自动勾选，导出 SRT 时才会真正附加名称和分隔符；这个导出选项仍可手动关闭。按颜色导出 SRT 时，还可以在“导出 → 颜色与说话人”中选择使用说话人名称作为文件名后缀。
