...
- **SRT / ASS / TXT** ： SRT 用于播放器、剪辑软件和普通字幕交付；可选择把首条字幕的起点拉到 0（只延长首条，不改变其结束时间或后续字幕时间码）。ASS 导出读取「管理 ASS 样式」中的默认输出方案及其关联样式；工程记录的源视频宽高会写入 `PlayResX` / `PlayResY`，字号按 `PlayResY / 1080` 换算。方案中的 `\fad`、`\fade`、`\move`、`\t` 会自动添加到每一条启用字幕。五种字幕颜色在 ASS 能表达为文字色或描边色时会写入对应样式；现有“下划线”颜色预览无法在 ASS 中独立表达，因此不会额外伪造。启用「导出时附加说话人名称」时还会把名称写入 ASS 事件的 `Name` 字段，并可用局部标签保持说话人颜色。工程存在颜色标记时，可导出完整字幕，或按每种已使用颜色（含无颜色的默认组，中文界面文件名后缀为「默认」，英文界面为 `default`）分别生成带颜色名后缀的 SRT。也可导出逐字幕行排列的纯文本 TXT。
- **SRT / ASS / TXT** ： SRT 用于播放器、剪辑软件和普通字幕交付；可选择把首条字幕的起点拉到 0（只延长首条，不改变其结束时间或后续字幕时间码）。ASS 使用主字幕预览当前选中的字体、字号和文字颜色写入样式；工程记录的源视频宽高会写入 `PlayResX` / `PlayResY`，字号以编辑器默认预览字号 `18px` 对应 1080p ASS `72` 为基准，再按 `PlayResY / 1080` 换算（例如 4K 视频会把默认字号写成约 `144`，手动 `32px` 写成约 `256`）。这个基准已包含全屏预览将字号放大到约两倍的视觉校准。位置、描边和背景采用通用底部居中样式，不写入预览框几何。五种字幕颜色分别写入 `YELLOW`、`GREEN`、`RED`、`PURPLE`、`BLUE` 样式，启用「导出时附加说话人名称」时还会把名称写入 ASS 事件的 `Name` 字段，并保留可见的文字前缀。工程存在颜色标记时，可导出完整字幕，或按每种已使用颜色（含无颜色的默认组，中文界面文件名后缀为「默认」，英文界面为 `default`）分别生成带颜色名后缀的 SRT。也可导出逐字幕行排列的纯文本 TXT。
...
位置与大小以归一化比例保存在工程文件的 `preview.subtitle`，因此播放器或窗口改变大小后仍保持相对位置。设置中的“字幕样式”可以选择主字幕和副字幕的字号、字体与基础颜色；“字幕颜色”则控制预览字幕的颜色样式，以及颜色到说话人名称的映射。波形设置中的“频谱颜色”默认关闭，关闭时使用纯色波形，打开后才使用频谱缓存着色。字号缺失时沿用响应式默认值；导出 ASS 时会以 1080p 的默认字号 `72` 为基准，按输出视频高度换算，并按相同比例处理手动字号。一次拖动或缩放只产生一条撤销记录；这些设置只改变预览呈现，绝不修改字幕或字词时间。
