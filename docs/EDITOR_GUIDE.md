...
独立叠加轨的字幕同样支持拆分与合并：面板拆分、叠加字幕右键菜单「拆分此叠加字幕」与剃刀工具都会打开拆分弹窗（与副轨同一套交互），`Ctrl/Cmd+Shift+A/D` 可粘合相邻叠加字幕；拆分与合并时颜色/表情包组按主轨同款规则继承或拆分（组引用始终指向叠加轨自身段）。
...
<<<<<<< HEAD
- **SRT / ASS / TXT**：SRT 用于播放器、剪辑软件和普通字幕交付；可选择把首条字幕的起点拉到 0（只延长首条，不改变其结束时间或后续字幕时间码）。ASS 使用主字幕预览当前选中的字体、字号和文字颜色写入默认样式，适合交给支持样式字幕的播放器或剪辑软件；位置、描边和背景采用通用底部居中样式，不写入预览框几何或每条字幕的颜色分组。工程开启独立叠加轨时，叠加字幕会以独立图层（Layer 1，顶部居中）一并写入 ASS，可与主轨字幕时间重叠。工程存在颜色标记时，可导出完整字幕，或按每种已使用颜色（含无颜色的 `default`）分别生成带颜色名后缀的 SRT——叠加轨字幕按自身颜色并入对应颜色的 SRT，说话人标签同样覆盖叠加轨。也可导出逐字幕行排列的纯文本 TXT。
...
- **FCP7 XML / OTIO 中的叠加轨**：工程开启独立叠加轨时，FCP7 XML 可把叠加字幕与主轨/副轨一样作为原生文本轨导出（选择「全部字幕轨」时各占一轨），叠加轨表情包与主轨表情包分别写入各自的图片轨；OTIO 会把叠加字幕写成独立的「叠加字幕」标记轨，叠加轨表情包写入独立的「叠加表情」视频轨，均与主轨内容分开。叠加轨关闭（`enabled: false`）时不参与导出。
