...
「批量操作 → 文稿粗剪」用于按已核对的主字幕做口播粗剪。一个母工程可新建、复制、重命名、切换和删除多套粗剪方案；各方案独立保存视频取舍、输出名称和 SRT 来源，但共用母稿文字与时间轴。窗口内可改文字，但只有逐条点击“剪掉”才会移除对应音视频；清空文字不会自动剪片。也可拖入从当前工程导出后删改的 SRT：程序先生成匹配报告，确认后取舍只作用于当前方案，文字修正写回共享母稿，存疑内容不自动剪。应用后可按当前方案播放跳过预览。Server 编辑器可导出当前方案，也可顺序批量导出全部有效方案；某一条失败不会中断后续导出。当前仍只支持单视频和整条主字幕，不做句内删词、重排、转场或字幕烧录。
