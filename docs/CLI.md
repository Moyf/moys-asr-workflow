...
- `--debug-raw` ：在线模型单独保存完整 ASR 原始返回；本地模型按引擎保存可用的原始/中间调试产物，并生成 `<输出名>.local-debug.json` 清单。清单旁会按阶段生成 `*.local-debug.*.json`，例如 FireRed 的 CTC 原始字词时间码、ct-punc 标点结果、最终断句结果；未指定 `-o` 时跟随默认 SRT 输出位置，指定了 `-o` 时与输出同目录。
