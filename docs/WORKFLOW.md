...
- 常规 SRT 通过工具栏导出；若启用了空隙移除，可选择去空隙 SRT、OTIO、视频/音频 EDL、FFconcat 或保留区域 JSON。
- 去空隙 EDL 是 30fps 非丢帧 CMX 3600 文件：视频媒体写入成对的 `V` / `AA` 源媒体事件，音频媒体写入 `AA` 事件；源入出点对应原媒体，记录入出点则连续拼接移除空隙后的时间线。EDL 只引用媒体路径，不包含媒体副本。
