...
默认保存到程序目录下的 `asr-presets/`（源码运行时为项目根目录）；该目录无法写入时回退到 MAW 用户数据目录的 `asr-presets/`。成功保存或载入后，两种操作共用并记住上次文件夹，重启后仍生效；取消和载入失败不会更新记录。预设使用普通 `.json` 后缀；文件内的 `schema` 标识用于识别预设格式，不依赖文件名。
...
自动处理会保留原始转写结果，最终结果另写为带操作后缀的 `.mosp` 和 `.srt`（中文界面为 `.后处理`，英文界面为 `.postprocess`）。失败或取消不会影响原始结果，并会保留中间目录供恢复。
...
- 批量 V1 会跳过「文稿匹配」步骤，避免把同一份文稿错误用于多个媒体；这个限制不会修改已保存的单文件文稿匹配设置。其他自动处理步骤仍按 [转写后自动处理](POSTPROCESS_PIPELINE.md) 的规则执行。

## 2.7 输出文件默认放在哪里

最终产物和自动生成的辅助文件默认分开存放：SRT / `.mosp`（以及之后导出的 ASS）留在媒体旁，其余波形 sidecar、转换缓存、HTML 等辅助文件归入媒体旁的 `_maw` 子目录，避免媒体文件夹被缓存堆满：
...
如果关联媒体是 FLV，服务器会优先复用已有转换缓存（旧版写在媒体旁的 `clip.mp4`，或 `_maw` 里的新缓存）；都没有时再调用用户配置的 `FFMPEG_PATH`，或 PATH 中的 `ffmpeg`，把转换结果原子写入媒体旁的 `_maw` 目录（`clip.flv` 对应 `_maw/clip.mp4`）。Desktop 版使用随应用提供的 FFmpeg sidecar。工程仍保存可继续使用的媒体路径。
...
「OCR 字幕去重」的完整使用说明见 [OCR_SUBTITLE_DEDUP.md](OCR_SUBTITLE_DEDUP.md)。
...
切换到「实用工具」后，页面只显示媒体操作：生成波形、媒体重组、烧录字幕和提取音频。它自己的「媒体文件」默认跟随 Launcher 当前媒体，但允许选择、拖入或直接输入独立覆盖值；清空该输入即可恢复跟随 Launcher。生成波形提供「生成波形文件」和「生成波形并打开编辑器」两个按钮，并可单独选择是否写入 reapeaks 频谱数据：前者只写出媒体专用 `.mosp`，后者还会把该工程切换为 Launcher 当前工程并打开编辑器。媒体重组同样只使用此页面的媒体文件，不会改写字幕时间轴或后处理的输入链；先在编辑器中执行「移除静音空隙」，导出 FFconcat 文件后可选择或拖入该文件重组媒体。
...
- 录音中出现文稿没有的完整 ASR 字幕段时，匹配会按原文字和时间将它们保留在对应文稿字幕之间，不会并入后一个文稿句。
- 匹配度低于安全阈值时不会写出文件；成功结果中的警告会列出匹配度和未匹配段。字幕/工程解析失败会单独提示输入格式或时间码问题，不会误报为低匹配度。
- 匹配面板默认开启「自动清理 Markdown 符号」，会在预览和匹配前移除 `**粗体**`、`*斜体*`、`~~删除线~~`、`` `行内代码` `` 以及 `==高亮==` 等成对标记，只保留可见文字；取消勾选即可保留这些符号。手动匹配与自动后处理共用此设置。
...
固定替换包含两部分：批量替换和简繁转换。批量替换每行填写一条 `原文 => 新文`，按从上到下的顺序应用；转换可选「不转换」「转为简体」或「转为繁体」，并在批量替换之后执行。它适合统一人名、产品名、固定错别字和字幕字形，不调用网络服务。处理只改变文字，分段起止时间保持不变；文字变化的段会移除旧逐词时间，避免文字与 `items` 不一致。自动处理管线中的固定替换位于翻译前。
