...
翻译任务在发送前会做目标语言预过滤：已是目标语言的字幕不发送给模型，按原位置逐字节保留（含逐词时间码；空字幕同样跳过）。判断是纯字符脚本启发式：中文按 CJK 表意文字占「中文 + 拉丁字母」的比例 ≥ 70% 视为已是中文，英文按「不含 CJK 字符」视为已是英文；出现假名、谚文或其他文字，或中英混合的句子，一律照常发送给模型。纯数字/标点字幕没有可翻译内容，两个方向都会跳过。全部非空字幕都已是目标语言时直接报错，不写出产物。
